...
CANN Runtime支持以下几类显式同步，调用此类接口后，主机线程会阻塞直到相关的任务执行完成。
...
**对于异步接口**，主机线程调用异步接口后仅代表下发任务，不代表任务执行成功，在任务未完成前，异步接口已向主机线程返回成功。用户需要显示调用以上Runtime提供的同步接口阻塞主机线程，等待任务完成，否则可能会导致训练或推理等业务异常、Device断链掉卡等未知情况。
