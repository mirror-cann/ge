...
当前第一批拆分里，`libge_python_pass_bridge.so` 先只依赖纯 Python `bootstrap / _bridge` 协议；未来 `_ge_pass_native.so` 落地后，再由 bridge 从同目录按同一 artifact set 继续加载 native helper。
...
- instance holder
- Python pass 实例
- callback 中创建的 `Graph` / `Node` / `MatchResult` / `NodeOutput` Python 包装对象
...
- `Graph` / `PassContext` / `MatchResult` / `NodeOutput` 等正式 Python wrapper 的构造与转换
- Python 侧直接 import 的 helper 与工厂接口
- 对 `ge.passes.base` 提供可选的 native-backed 对象来源
...
- `api/python/ge/ge/passes/match_result.py`
- `api/python/ge/ge/passes/bootstrap.py`
- `api/python/ge/ge/passes/runtime.py`
...
- wrapper 来源层
- `PassContext` / `MatchResult` 等对象优先从 `_ge_pass_native` 取得 native-backed 实现
- 当 `_ge_pass_native` 尚未交付或本地 bring-up 验证时，允许 `base.py` 保留兼容 shim 或 placeholder，以便 bridge 拆分可先行联调
...
- `base.py` 中 `PassContext` / `MatchResult` 的 native-backed 接入与兼容导出
...
- `PassContext` / `MatchResult` 优先 re-export 或包装 `_ge_pass_native` 提供的类型
- 仅在 `_ge_pass_native` 缺失的开发联调场景下保留兼容 shim

A 在本地拆分 bridge `.so` 与 loader 时，可先不依赖 `_ge_pass_native`，采用以下临时验证策略：
