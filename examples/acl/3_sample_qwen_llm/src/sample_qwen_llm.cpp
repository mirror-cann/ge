...
void SampleQwenLlm::PrintInputs(size_t count) {
  ...
  for (size_t i = 0; i < inputNum && i < count; i++) {
    ...
    INFO_LOG("  Input[%zu], tensorName=%s, size=%zu bytes, dtype=%d, format=%d, dims=%s", i, tensorName, tensorSize,
             dataType, format, dimsOss.str().c_str());
  }
}
...
void SampleQwenLlm::PrintOutputs(size_t count, size_t lastPos) {
  ...
  for (size_t i = 0; i < outputNum && i < count; ++i) {
    ...
    INFO_LOG("  Output[%zu], tensorName=%s, size=%zu bytes, dtype=%d, format=%d, dims=%s", i, tensorName, tensorSize,
             dataType, format, dimsOss.str().c_str());
  }
...
Result SampleQwenLlm::Process() {
  ...
  InputTensor input0{.data = inputIds.data(), .byte_size = inputIds.size() * sizeof(int64_t)};
