...
Pattern 是一个标准的 ge::Graph，其拓扑结构就是你想在目标图中找到的子图结构。节点类型对应算子类型，边对应数据流向。
...
| 角色            | 用法               | 匹配行为       |                                                                                                                                     
|:--------------|:-----------------|:----------------|
| Data 节点       | 子图的外部输入占位符       | 无条件匹配，不限类型      |  
| 普通算子节点        | 要融合的算子           | 精确匹配 op type    |                                                                                                                                          
| NetOutput 节点  | 声明子图的输出          | 框架自动添加          |
...
| 边界           | 定义                                           | 行为                                            |
|:--------------|:---------------------------------------------|:----------------------------------------------|
| 输入边界 | 用 Data 节点占位。凡是来自子图外部的 tensor，用一个 Data 节点代表它。 | Data 节点默认匹配任意节点，相当于"通配符输入"                    |
|输出边界| 连到 NetOutput 的才是输出，哪个节点的输出需要被子图外部消费，该 output 就必须连到 NetOutput                          | 只有连到 NetOutput 的节点的对应 output，才会作为融合后子图的输出暴露出去 |

---                                                                                                                                                                                                      
四、自包含性约束（最容易踩坑，需要正确理解）

子图替换是一次接口替换：框架将匹配到的子图整体移除，插入替换算子，再按 Pattern 声明的输入/输出重新接线。
所有"出口"必须事先在 pattern 中声明。未声明的内部 tensor，替换后无法向外界交付，其消费者的输入连接将断裂。
                  

▎ 对于非输出节点，其所有下游消费者都必须在子图内部。
违反自包含且改图成功时会发生什么？

假设内部节点 X 的同一个输出tensor有外部消费者 B，但 X 未被声明为输出：
...
替换算子只知道自己需要暴露哪些输出（由 SubgraphBoundary 的 outputs 决定）。X 不在 outputs 中，替换算子不会为 X 的 tensor 预留输出口。替换完成后，B 的输入 tensor 凭空消失 —— 图的连通性被破坏。

对于pattern定义者要如何做：

场景①：普通内部节点，输出只在 pattern 内消费                                                                                                                                                             
正常定义，不需要连 NetOutput。

Data → X → A → NetOutput

场景②：某节点的输出，部分在 pattern 内消费，部分在 pattern 外消费                                                                                                                                        
必须将该节点的 output 也连到 NetOutput，否则匹配会被拒绝。

Data → X → A → NetOutput[0]   ← A 作为主输出                                                                                                                                                             
       ↓                                                                                                                                                                                             
NetOutput[1]              X 的输出也要暴露，供外部消费者重连

如果漏掉这一步，框架检测到 X 有外部消费者但不是输出节点，会直接丢弃该次匹配。
                                                                                                                                                                                                           
---                                                                                                                                                                                                      
五、输入边数必须精确匹配

算子节点的输入个数严格对齐。若目标图中算子有 3 个输入，pattern 中对应节点也必须有 3 个输入（可以是 Data 占位）。
...
|:-----------------|:--------------------------|                                                                                                                                               
|控制边            | 框架不支持控制依赖匹配     | 
|子图（subgraph） | 不支持嵌套子图匹配        |                                                                                                                                
|动态输入/输出节点 |无法确定输入个数，匹配歧义|
                  
---                                                                                                                                                                                                      
七、多输出 Pattern 的匹配顺序

当 NetOutput 有多个输入（即 pattern 有多个输出）时，框架以 NetOutput[0] 的 producer 作为主锚点遍历目标图候选，其余输出的匹配结果必须与主锚点找到的节点一致。
→ 建议将约束性最强（目标图中数量最少）的算子放在 NetOutput[0]，以减少无效回溯。
...
- 每个外部输入都有 Data 节点占位？
- 所有需要在融合后继续被外部使用的 output 都连到了 NetOutput？
- 每个算子的输入边数与目标图中一致？
- Pattern 中没有控制边、子图、动态 I/O？
