...
aclError AddBuffersToDataset(aclmdlDesc *model_desc, aclmdlDataset *dataset, bool is_input,
                             std::vector<void *> &dev_ptrs, std::vector<size_t> &sizes) {
  size_t num = is_input ? aclmdlGetNumInputs(model_desc) : aclmdlGetNumOutputs(model_desc);
  for (size_t i = 0; i < num; ++i) {
    size_t size = is_input ? aclmdlGetInputSizeByIndex(model_desc, i) : aclmdlGetOutputSizeByIndex(model_desc, i);
    void *dev = nullptr;
    aclError err = aclrtMalloc(&dev, size, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (err != ACL_SUCCESS) {
      std::cerr << "[Error] aclrtMalloc failed for " << (is_input ? "input" : "output") << " " << i << std::endl;
...
aclError CreateBuffers(aclmdlDesc *model_desc, ModelBuffers &buffers) {
  ...
  if (AddBuffersToDataset(model_desc, buffers.input_dataset, true, buffers.input_dev_ptrs, buffers.input_sizes) !=
      ACL_SUCCESS) {
    ReleaseBuffers(buffers);
    return ACL_ERROR_INTERNAL_ERROR;
  }
  if (AddBuffersToDataset(model_desc, buffers.output_dataset, false, buffers.output_dev_ptrs, buffers.output_sizes) !=
      ACL_SUCCESS) {
    ReleaseBuffers(buffers);
    return ACL_ERROR_INTERNAL_ERROR;
  }
...
aclError LoadModelFromFile(const std::string &om_path, uint32_t &model_id, aclmdlDesc *&model_desc) {
  ...
  if (err != ACL_SUCCESS) {
    std::cerr << "[Error] Failed to load model from " << om_path << ", error=" << err << std::endl;
    return err;
  }
