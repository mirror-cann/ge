...
python/
├── src/
|   └── make_pfa_hcom_graph.py     // sample file
├── rank_table/
|   ├── a2/
...
4. Run TP graph in parallel on 2 NPU devices (device ID automatically read from rank_table)
...
HcomAllReduce operator prototype is shown below, ES graph construction generated API is `HcomAllReduce()`, supporting use in Python
...
Note: reduction and group are required keyword parameters, fusion and fusion_id are optional parameters with default values
