...
void Profiler::RegisterString(const int64_t index, const std::string &str) {
  ...
  const auto ret =
      strncpy_s(PtrAdd<StrHash>(GetStringHashes(), static_cast<size_t>(kMaxStrIndex), static_cast<size_t>(index))->str,
...
void Profiler::DumpByIndex(const int64_t index, std::ostream &out_stream) const {
  if ((index < 0) || (index >= kMaxStrIndex) ||
      (strnlen(
           PtrAdd<const StrHash>(GetStringHashes(), static_cast<size_t>(kMaxStrIndex), static_cast<size_t>(index))->str,
           kMaxStrLen) == 0UL)) {
