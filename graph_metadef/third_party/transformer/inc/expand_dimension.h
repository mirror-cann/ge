...
class ExpandDimension {
 public:
  ...
  static int64_t GetReshapeAxicValue(const int64_t &reshape_type_mask, const ge::GeShape &shape, int32_t axis_index);
  static int64_t GetReshapeAxicValueByName(const int64_t &reshape_type_mask, char ch, const ge::GeShape &shape,
                                           const ge::Format &format);
  static bool GetFormatFullSize(const ge::Format &format, size_t &full_size);

 private:
  static bool IsNeedExpand(const ge::Format &origin_format, const ge::Format &format, const size_t &origin_dim_size,
