...
Status DataTypeTransfer::TransDataType(const CastArgs &args, TransResult &result) {
  ...
  if (iter == trans_mode_map.end()) {
    const std::string error =
        "Failed to trans data from datatype " + FmtToStr(TypeUtils::DataTypeToSerialString(args.src_data_type)) +
        " to " + FmtToStr(TypeUtils::DataTypeToSerialString(args.dst_data_type)) + " , it is not supported.";
