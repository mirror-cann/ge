...
class ErrorTracking {
 public:
  ...
  void UpdateTaskId(const uint32_t old_task_id, const uint32_t new_task_id, const uint32_t stream_id,
                    const uint32_t model);
 ...
 private:
  ErrorTracking();
  void AddTaskOpdescInfo(const OpDescPtr &op, const TaskKey &key, std::map<TaskKey, ErrorTrackingOpInfo> &map,
