...
bool ErrorTracking::GetGraphTaskOpdescInfo(const uint32_t task_id, const uint32_t stream_id,
...
bool ErrorTracking::GetSingleOpTaskOpdescInfo(const uint32_t task_id, const uint32_t stream_id,
                                              ErrorTrackingOpInfo &op_info) {
  TaskKey key(task_id, stream_id);
...
void ErrorTracking::UpdateTaskId(const uint32_t old_task_id, const uint32_t new_task_id, const uint32_t stream_id,
                                 const uint32_t model) {
  ...
  if (it != task_map.end()) {
    ...
    GELOGD("Update task id, old: %u -> new: %u, stream_id: %u, model: %u, opname: %s", old_task_id, new_task_id,
           stream_id, model, opname.c_str());
...
void ErrorTrackingCallback(rtExceptionInfo *const exception_data) {
  ...
  if ((exception_data->retcode == ACL_ERROR_RT_AICORE_OVER_FLOW) ||
      (exception_data->retcode == ACL_ERROR_RT_AIVEC_OVER_FLOW) ||
      (exception_data->retcode == ACL_ERROR_RT_OVER_FLOW)) {
    return;
  }
  GELOGI("ErrorTracking callback in, task_id %u, stream_id %u.", exception_data->taskid, exception_data->streamid);
  ErrorTrackingOpInfo op_info;
  bool founded = false;
  if (exception_data->expandInfo.type == RT_EXCEPTION_FFTS_PLUS) {
    ...
    founded =
        ErrorTracking::GetInstance().GetGraphTaskOpdescInfo(exception_data->taskid, exception_data->streamid, op_info);
    if (!founded) {
      founded = ErrorTracking::GetInstance().GetSingleOpTaskOpdescInfo(exception_data->taskid, exception_data->streamid,
                                                                       op_info);
    }
  }
  if (founded) {
    ...
    REPORT_INNER_ERR_MSG("E18888",
                         "Op execute failed. origin_op_name [%s], op_name [%s], "
                         "error_info: task_id %u, stream_id %u, tid %u, device_id %u, retcode 0x%x",
                         op_info.op_origin_name.c_str(), op_info.op_name.c_str(), exception_data->taskid,
                         exception_data->streamid, exception_data->tid, exception_data->deviceid,
                         exception_data->retcode);
  }
}
