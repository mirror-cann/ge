...
class TensorTransUtils {
 public:
  ...
  static Status TransHostGertTensorsToDevice(Allocator *allocator, const std::vector<gert::Tensor> &src_tensors,
                                             std::vector<gert::Tensor> &dst_tensors,
                                             std::vector<MemBlock *> &inputs_memblocks, bool enable_input_batch_cpy);
  static Status HostTensorToDeviceGertTensor(ge::Allocator *allocator, const void *src_tensor_addr,
  ...
  static Status GeTensors2GertTensors(const std::vector<GeTensor> &ge_tensors, std::vector<gert::Tensor> &gert_tensors);

  // Tensor -> gert::Tensor
  static Status Tensor2GertTensor(const Tensor &ge_tensor, gert::Tensor &gert_tensor);
  static Status Tensors2GertTensors(const std::vector<Tensor> &ge_tensors, std::vector<gert::Tensor> &gert_tensors);
  ...
  static Status AsTensorsView(const std::vector<Tensor> &ge_tensors, std::vector<gert::Tensor> &tensors_view);
