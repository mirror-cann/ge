...
Status DSATaskCodeBuilder::Contribute(TaskSemanticContributeContext &context) {
  ...
  GELOGI("DSA Task Codegen: op[%s], sqe_type[%u], stream_id[%u].", header_.op_name.c_str(), dsa_sqe_.sqe_type,
         header_.stream_id);
  return SUCCESS;
}
...
void DSATaskCodeBuilder::RenderAddrLowHigh(const ExprRef &sqe_attr_low, const ExprRef &sqe_attr_high,
  ...
  (void)items.push_back(
      ast_.Assign(sqe_attr_low,
                  ast_.StaticCast("uint32_t", ast_.ReinterpretCast("uintptr_t", addr_expr) & ast_.UInt(kMask32Bits))));
  (void)items.push_back(
      ast_.Assign(sqe_attr_high, ast_.StaticCast("uint32_t", ast_.ReinterpretCast("uintptr_t", addr_expr) >> k32Bits)));
}
...
void DSATaskCodeBuilder::RenderSqeAddrFields(const VarRef &sqe_var, std::vector<BodyItem> &items) {
  ...
  RenderAddrLowHigh(sqe_var.Attr("dsaCfgStateAddrLow"), sqe_var.Attr("dsaCfgStateAddrHigh"), state_addr, items);

  // 3. dsaCfgParamAddr
  auto param_addr_var = ast_.Var("uint64_t", "op" + std::to_string(header_.op_index) + "_dsa_param_addr");
  (void)items.push_back(ast_.VarDecl(
      param_addr_var,
      ast_.StaticCast(
          "uint64_t",
  ...
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("dsaCfgParamAddrHigh"), ast_.StaticCast("uint32_t", param_addr_var >> k32Bits)));
}
...
void DSATaskCodeBuilder::RenderSqeVarFields(const VarRef &sqe_var, std::vector<BodyItem> &items) {
  ...
  if (dsa_sqe_.seed_is_addr) {
    (void)items.push_back(
        ast_.VarDecl(seed_var, ast_.Var("auto", input_addrs_[1U].symbol_hint).Attr("device_address")));
  } else {
    (void)items.push_back(ast_.VarDecl(seed_var, ast_.UInt(dsa_sqe_.seed_value)));
  }
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("dsaCfgSeedLow"), ast_.StaticCast("uint32_t", seed_var & ast_.UInt(kMask32Bits))));
  (void)items.push_back(ast_.Assign(sqe_var.Attr("dsaCfgSeedHigh"), ast_.StaticCast("uint32_t", seed_var >> k32Bits)));

  // 5. dsaCfgNumber
  auto count_var = ast_.Var("uint64_t", "op" + std::to_string(header_.op_index) + "_dsa_count");
  if (dsa_sqe_.random_count_is_addr) {
    (void)items.push_back(
        ast_.VarDecl(count_var, ast_.Var("auto", input_addrs_[0U].symbol_hint).Attr("device_address")));
  } else {
    (void)items.push_back(ast_.VarDecl(count_var, ast_.UInt(dsa_sqe_.random_count_value)));
  }
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("dsaCfgNumberLow"), ast_.StaticCast("uint32_t", count_var & ast_.UInt(kMask32Bits))));
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("dsaCfgNumberHigh"), ast_.StaticCast("uint32_t", count_var >> k32Bits)));
}
...
void DSATaskCodeBuilder::RenderHbmArgsCopy(const VarRef &sqe_var, std::vector<BodyItem> &items) {
  ...
  auto ioaddr_var = ast_.Var("std::vector<uint64_t>", "op" + std::to_string(header_.op_index) + "_dsa_iow_addr");
  (void)items.emplace_back(ast_.VarDecl(ioaddr_var, FlattenHostArgs(io_args_vars)));

  (void)items.push_back(ChkStatus(
      MemcpyS(args_table_.Attr("GetArgsInfo")(static_cast<int64_t>(hbm_entry_->table_index)).Arrow("host_addr"),
  ...
  (void)items.push_back(ChkStatus(
      AclrtMemcpy(args_table_.Attr("GetArgsInfo")(static_cast<int64_t>(hbm_entry_->table_index)).Arrow("dev_addr"),
                  hbm_entry_->args_size, ioaddr_var.Data(), ioaddr_var.Size() * ast_.Sizeof("uint64_t"),
  ...
  (void)items.emplace_back(ast_.VarDecl(dump_flag_var, is_data_dump_var * ast_.UInt(2U)));

  (void)items.push_back(ChkStatus(ast_.Call(
      "KernelDsaTaskDistribute", {sqe_var.Ref().Addr(), ast_.StaticCast("uint32_t", ast_.Sizeof("rtStarsDsaSqe_t")),
                                  stream_list_[static_cast<int32_t>(header_.stream_id)], dump_flag_var})));
...
Status DSATaskCodeBuilder::RenderDistribution(std::vector<BodyItem> &items) {
  ...
  for (auto &addr : output_addrs_) {
    ...
    (void)items.push_back(ast_.VarDecl(
        ...
        ast_.Call("BuildOm2Tensor",
                  {GetAddr(total_dev_mem_ptr_, addr.mem_offset), ast_.ULong(tensor_info.size), tensor_info.data_type,
                   tensor_info.format, ast_.Var("std::vector<int64_t>", shape_var_name).Data(),
                   ast_.Var("std::vector<int64_t>", shape_var_name).Size()})));
  }
  for (auto &addr : workspace_addrs_) {
    if (!addr.is_reused_from_upstream) {
      auto &base_ptr =
          (addr.memory_type == (kSessionScopeMemoryMask | RT_MEMORY_HBM)) ? session_scope_mem_ptr_ : total_dev_mem_ptr_;
      (void)items.push_back(ast_.VarDecl("auto", addr.symbol_hint, GetAddr(base_ptr, addr.mem_offset)));
    }
  }
  ...
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("sqeHeader").Attr("type"), ast_.UInt(static_cast<uint64_t>(dsa_sqe_.sqe_type))));
  (void)items.push_back(ast_.Assign(sqe_var.Attr("start"), ast_.UInt(static_cast<uint64_t>(dsa_sqe_.start))));
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("functionType"), ast_.UInt(static_cast<uint64_t>(dsa_sqe_.distribution_type))));
  ...
  (void)items.push_back(
      ast_.Assign(sqe_var.Attr("paramVldBitmap"), ast_.UInt(static_cast<uint64_t>(dsa_sqe_.param_vld_bitmap))));
  (void)items.push_back(ast_.Assign(sqe_var.Attr("paramAddrValBitmap"),
                                    ast_.UInt(static_cast<uint64_t>(dsa_sqe_.param_addr_val_bitmap))));
