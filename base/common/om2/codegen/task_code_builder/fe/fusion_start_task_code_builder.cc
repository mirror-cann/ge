...
Status FusionStartTaskCodeBuilder::RenderDistribution(std::vector<BodyItem> &items) {
  auto stream_list = ast_.Var("std::vector<aclrtStream>", "stream_list_");
  items.push_back(ast_.Comment("============================= FUSION_START ==============================="));
  items.push_back(ChkStatus(ast_.Call("KernelFusionStartDistribute", {stream_list[header_.stream_id]})));
  return SUCCESS;
}
