...
MixTaskPara CalcMixTaskParaByType(string &core_type, int64_t block_dim, int64_t ai_core_num, int64_t vec_core_num,
                                  int64_t all_core_num, int64_t ratio) {
  MixTaskPara para;
  FE_LOGD("Before calculating core type[%s] blk[%ld] ai_core[%ld] vec_core[%ld] all_core[%ld].", core_type.c_str(),
          block_dim, ai_core_num, vec_core_num, all_core_num);
...
Status FillMixExtraTask(const ge::Node &node, string &core_type, MixTaskPara &para, domi::TaskDef &main_task,
  ...
  if (notify_id_v.size() != kMixNotifyIdNum) {
    FE_LOGW("Op[%s][%s] Get notify id size: %zu is not 2, switching to normal mode.", node.GetNamePtr(),
            node.GetTypePtr(), notify_id_v.size());
    return SUCCESS;
  }
  ...
  if (UpdateMixAiCoreTask(para, main_task, sub_task) != SUCCESS) {
    REPORT_FE_ERROR("[GenTask][FillMixExtraTask] Op[%s][%s] Update task definition failed.", node.GetNamePtr(),
                    node.GetTypePtr());
    return FAILED;
  }
...
Status GenerateMixTask(const ge::Node &node, std::vector<domi::TaskDef> &task_defs) {
  ...
  if (ai_task.type() != ACL_RT_MODEL_TASK_KERNEL && ai_task.type() != ACL_RT_MODEL_TASK_ALL_KERNEL) {
    REPORT_FE_ERROR("[GenTask][GenerateMixTask] Op[%s][%s] did not find ai core task.", node.GetNamePtr(),
                    node.GetTypePtr());
    return FAILED;
  }
...
Status TaskBuilder::GenerateKernelTask(const ge::Node &node, TaskBuilderContext &task_context,
  ...
  if (GenerateMixTask(node, task_defs) != SUCCESS) {
    REPORT_FE_ERROR("[GenTask][GenerateMixTask] Op[%s][%s] failed to generate mix task.", node.GetNamePtr(),
                    node.GetTypePtr());
    return FAILED;
  }
...
Status TaskBuilder::GenTaskForOneNode(const ge::Node &node, TaskBuilderContext &task_context,
  ...
  if (memset_node != nullptr) {
    ...
    if (DoGenerateTask(*memset_node, task_context, mem_set_task_def) != SUCCESS) {
      REPORT_FE_ERROR("[GenTask][MemsetGen][Node %s, type %s] generate task failed!", memset_node->GetName().c_str(),
    ...
    FE_LOGD("Generate task for memset node[%s, %s] of op[%s, %s].", memset_node->GetName().c_str(),
            memset_node->GetType().c_str(), node.GetName().c_str(), node.GetType().c_str());
  }
...
Status TaskBuilder::DoGenerateTask(const ge::Node &node, TaskBuilderContext &task_context, domi::TaskDef &task_def) {
  ...
  if (GenerateOpArgsFormat(node.GetOpDesc(), args_format_str) != SUCCESS) {
    REPORT_FE_ERROR("[GenTask][InitKernelTask] Node [%s][%s]: Generating args format failed.", node.GetNamePtr(),
                    node.GetTypePtr());
  ...
  if (task_adapter == nullptr) {
    REPORT_FE_ERROR("[GenTask][DoGenerateTask][Node %s type %s] Failed to create adapter.", node.GetName().c_str(),
                    node.GetType().c_str());
  ...
  if (status != SUCCESS) {
    REPORT_FE_ERROR("[GenTask][DoGenerateTask][Node %s type %s] Failed to run task adapter.", node.GetName().c_str(),
                    node.GetType().c_str());
    return status;
  }
  ...
  if (status != SUCCESS) {
    REPORT_FE_ERROR("[GenTask][DoGenerateTask][Node %s type %s] Failed to fill up task def.", node.GetName().c_str(),
                    node.GetType().c_str());
    return status;
  }
  return SUCCESS;
...
Status TaskBuilder::GenerateOpArgsFormat(const ge::OpDescPtr &op_desc, std::string &args_format_str) {
  ...
  if (args_construct.ConstructNodeArgsDesc() != SUCCESS) {
    FE_LOGE("[GenTask][DoGenerateTask][Node %s type %s] Construct args desc failed.", op_desc->GetNamePtr(),
            op_desc->GetTypePtr());
    return FAILED;
  }
  args_format_str = args_construct.GetArgsFormatString();
  FE_LOGD("Node[%s][%s] generate args format %s.", op_desc->GetNamePtr(), op_desc->GetTypePtr(),
          args_format_str.c_str());
  return SUCCESS;
}
  ...
  if (task_def.type() == ACL_RT_MODEL_TASK_ALL_KERNEL) {
    ...
    if (ge::AttrUtils::GetStr(op_desc, ATTR_NAME_KERNEL_LIST_FIRST_NAME, first_kernel_name)) {
      ...
      FE_LOGD("Setting node_info[%s] and original_kernel_key[%s] for the kernel_def_with_handle of node[%s, %s].",
              first_kernel_name.c_str(), attr_val_kernel_name.c_str(), op_desc->GetName().c_str(),
...
TaskBuilderAdapterPtr TaskBuilder::CreateTaskAdapter(const ge::Node &node, TaskBuilderContext &task_context) {
  ...
  FE_LOGD("Create TaskBuilderAdapter success. OpName:%s, OpType:%s.", op_desc->GetName().c_str(),
          op_desc->GetType().c_str());
...
void TaskBuilder::SetSlowContextSwitchAttr(const ge::OpDescPtr &op_desc) {
  ...
  if (ge::AttrUtils::GetListInt(op_desc, ge::ATTR_NAME_OUTPUT_MEM_TYPE_LIST, output_mem_types)) {
    ...
    if (iter_l1 != output_mem_types.end() || iter_l2 != output_mem_types.end()) {
      ...
      FE_LOGD("Set switch buffer type[%s] for op[%s, %s].", kSwitchBufferTypeUB.c_str(), op_desc->GetName().c_str(),
              op_desc->GetType().c_str());
    }
  }
}

void TaskBuilder::GenerateCMOTask(const ge::Node &node, const bool pre_cmo_task, TaskBuilderContext &task_context,
                                  std::vector<domi::TaskDef> &task_defs) {
