...
const std::vector<GraphFusionPassType> GRAPH_FUSION_QUANT_PASS_VEC = {BUILT_IN_TF_MERGE_SUB_GRAPH_PASS,
                                                                      BUILT_IN_QUANT_OPTIMIZATION_GRAPH_PASS,
                                                                      BUILT_IN_EN_ISA_ARCH_EXC_V300_AND_V220_GRAPH_PASS,
                                                                      BUILT_IN_EN_ISA_ARCH_V100_GRAPH_PASS,
                                                                      BUILT_IN_EN_ISA_ARCH_V200_GRAPH_PASS,
...
struct FusionPassOrRule {
  ...
  FusionPassOrRule(std::string name_param, int32_t type_param, std::string method_param, int32_t priority_param,
...
struct BufferFusionInfo {
  ...
  BufferFusionInfo(const std::string &name_param, const int32_t priority_param, const bool is_auto_fusion_param,
                   const bool is_fusion_check_param,
                   BufferFusionPassRegistry::CreateFn buffer_fusion_pass_create_fn_param)
      : name(name_param),
        priority(priority_param),
...
class FusionPriorityManager {
 public:
  ...
  bool GetFusionSwitchByName(const std::string &pass_name, const std::string &pass_type, const uint64_t pass_attr = 0,
                             const bool can_close_fusion = false) const;
 ...
 private:
  ...
  Status InitCustomPasses(const bool &is_single_op_scene, vector<FusionPassOrRule> &custom_pass_or_rule_vec) const;

  Status InitCustomRules(vector<FusionPassOrRule> &custom_pass_or_rule_vec) const;

  Status InitBuiltInPasses(const bool &is_single_op_scene, vector<FusionPassOrRule> &built_in_pass_or_rule_vec) const;
  ...
  Status GetGraphFusionPassInfosByType(const GraphFusionPassType &pass_type, const bool &is_single_op_scene,
  ...
  Status GetBufferFusionPassInfosByType(const BufferFusionPassType &pass_type, const bool &can_close_buffer_fusion,
