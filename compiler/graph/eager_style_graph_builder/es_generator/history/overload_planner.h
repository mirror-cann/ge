...
class OverloadPlanner {
 ...
 private:
  ...
  enum class MultiBaselineMode { kTry0, kA1, kA2 };

  // 组装版本链：history.proto_chain + current。
  std::vector<const IrOpProto *> BuildVersionChain(const IrOpProto &current, const HistoryContext &history) const;
  ...
  std::vector<size_t> CollectBaselineIndices(size_t version_count, const std::vector<BoundaryInfo> &boundaries) const;
  // 在指定模式下构建一组签名并做二义性判定，成功则写入 accepted_signatures。
  bool TryAdoptModeSignatures(const IrOpProto &current, const std::vector<const IrOpProto *> &versions,
                              const std::vector<BoundaryInfo> &boundaries, MultiBaselineMode mode,
                              std::vector<Warning> *warnings, std::vector<Signature> &accepted_signatures) const;
  // 执行 Try0 -> A1 -> A2 的升级链；任一模式可用即返回 true。
  bool TryResolveModeEscalation(const IrOpProto &current, const std::vector<const IrOpProto *> &versions,
                                const std::vector<BoundaryInfo> &boundaries, const std::string &warning_context,
  ...
  std::vector<Signature> BuildMultiBaselineSignatures(const IrOpProto &current,
                                                      ...
                                                      MultiBaselineMode mode, std::vector<Warning> *warnings) const;
  // 计算单个 baseline 在 mode 下的强制 required/TensorHolder 策略与需裁剪输入。
  // baseline 负责保留哪一代调用外形, mode 负责这代外形里新增输入如何约束（Try0/A1/A2)
  BaselinePlanInput BuildBaselinePlanInput(const IrOpProto &current, const IrOpProto &baseline,
                                           const std::vector<BoundaryInfo> &boundaries, size_t baseline_index,
                                           MultiBaselineMode mode) const;
  // 基于边界新增输入追加 guard 删除签名（用于编译期阻断高风险调用形态）。
  void AppendBoundaryGuards(std::vector<Signature> &signatures, const std::vector<Signature> &baseline_signatures,
                            const std::vector<bool> &has_baseline_signature,
                            const std::vector<BoundaryInfo> &boundaries, MultiBaselineMode mode) const;
  ...
  Signature BuildSignature(const IrOpProto &current, const std::vector<std::string> *force_required_inputs,
                           const std::vector<std::string> *force_tensor_holder_inputs,
                           const std::vector<std::string> *removed_inputs, std::vector<Warning> *warnings) const;
  // 构造“删除签名 + deprecate 消息”的 guard，用于限制危险调用。
  Signature BuildNullptrGuardSignature(const Signature &sig, const std::string &input_name,
                                       bool tensor_holder_mode) const;
  int FindInputParamIndex(const Signature &sig, const std::string &input_name) const;
  void AppendInputParams(const IrOpProto &current, const DefaultValuePolicy &policy,
                         const std::vector<std::string> *force_tensor_holder_inputs, Signature &sig) const;
  ...
  void AppendOwnerBuilderParam(const IrOpProto &current, const DefaultValuePolicy &policy,
