...
#define DEFINE_WEAK_OP_FUNCTION(op_name)                                                                              \
  WEAK_SYMBOL EsCTensorHolder *Es##op_name(const EsCTensorHolder *x1, const EsCTensorHolder *x2) {                    \
    ...
    GE_ASSERT(false,                                                                                                  \
              ...
              "- Use: -Wl,--whole-archive libes_generated.a (name of the library, may be different from the codegen " \
              "library) -Wl,--no-whole-archive\n"                                                                     \
              "- Use: -Wl,--no-as-needed libes_generated.so (name of the library, may be different from the codegen " \
              "library) -Wl,--as-needed\n"                                                                            \
...
Status EsTensorHolder::AddControlEdge(const std::vector<EsTensorHolder> &ctrl_ins) const {
  GE_ASSERT_NOTNULL(tensor_holder_);
  return EsAddControlEdge(tensor_holder_, TensorsToEsCTensorHolders(ctrl_ins).data(),
                          static_cast<int64_t>(ctrl_ins.size()));
}
}  // namespace ge::es
