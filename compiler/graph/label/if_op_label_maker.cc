...
Status IfOpLabelMaker::Run(uint32_t &label_index) {
  ...
  if (then_branch_name.empty() || else_branch_name.empty()) {
    REPORT_INNER_ERR_MSG("E19999",
                         "Node:%s(%s), check subgraph invalid, "
                         "then branch graph: %s, else branch graph: %s",
                         if_desc->GetName().c_str(), if_desc->GetType().c_str(), then_branch_name.c_str(),
                         else_branch_name.c_str());
  ...
  if (then_stream_active == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add StreamActive node in graph:%s fail", then_sub_graph->GetName().c_str());
  ...
  if (then_enter_label == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelSetEnter node in graph:%s fail", then_sub_graph->GetName().c_str());
  ...
  if (AddLabelGotoLeave(then_sub_graph, then_leave_name, else_leave_index) == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelGotoLeave node in graph:%s fail", then_sub_graph->GetName().c_str());
  ...
  if (else_stream_active == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add StreamActive node in graph:%s fail", else_sub_graph->GetName().c_str());
  ...
  if (AddLabelSetEnter(else_sub_graph, else_enter_name, else_enter_index, else_stream_active) == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelSetEnter node in graph:%s fail", else_sub_graph->GetName().c_str());
  ...
  if (AddLabelSetLeave(else_sub_graph, else_leave_name, else_leave_index) == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelSetLeave node in graph:%s fail", else_sub_graph->GetName().c_str());
  ...
  if (switch_node == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelSwitchEnter node in graph:%s fail", then_sub_graph->GetName().c_str());
  ...
  if (AddLabelSwitchIndex(then_sub_graph, data_name, pred_desc, switch_node, parent_index) == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add LabelSwitchIndex node in graph:%s fail", then_sub_graph->GetName().c_str());
