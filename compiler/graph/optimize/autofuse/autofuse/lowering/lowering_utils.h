...
class LoweringUtils {
 public:
  ...
  static graphStatus CheckSpecialFuseType(loop::KernelBox &kernel_box,
...
class GraphFusionReasonStore {
 public:
  enum class FailReasonCategory { NODE_INFO_ERROR = 0, BACKEND_NOT_SUPPORTED = 1, TEMPORARILY_NOT_SUPPORTED = 2 };
  ...
  static void CountNodeFuseFailReason(const std::string &node_name, const std::string &reason,
 ...
 private:
  ...
  static const char *GetCategoryName(FailReasonCategory category) {
    switch (category) {
      case FailReasonCategory::NODE_INFO_ERROR:
        return "Node Info Incorrect";
      case FailReasonCategory::BACKEND_NOT_SUPPORTED:
        return "Backend Not Support";
      case FailReasonCategory::TEMPORARILY_NOT_SUPPORTED:
        return "Temporary Skip Lowering";
      default:
