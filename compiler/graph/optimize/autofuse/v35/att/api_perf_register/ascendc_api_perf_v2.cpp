...
ge::Status ExpandLoadBlockLen(const NodeDetail &node_info, PerfOutputInfo &perf, std::vector<Expr> &dims) {
  ...
  if (TryInitStaticExpand(node_info, dims, static_res) == ge::SUCCESS) {
    ...
    auto need_expand_checker = ge::sym::LogicalAnd(
        {ge::sym::Gt(node_info.gm_stride, CreateExpr(0)), ge::sym::Lt(block_len_ref_actual, cache_line_ele_num_threshold)});
...
ge::Status ExpandMTE3BlockLen(const NodeDetail &node_info, PerfOutputInfo &perf, std::vector<Expr> &dims) {
  ...
  if (TryInitStaticExpand(node_info, dims, static_res) == ge::SUCCESS) {
    ...
    auto need_padding_checker = ge::sym::LogicalAnd(
        {ge::sym::Gt(node_info.gm_stride, CreateExpr(0)), ge::sym::Lt(block_len_bytes, CreateExpr(static_res.cache_line_size))});
