...
template<typename T>
__simd_callee__ inline void CommonCalInTri(
...
template<typename T>
__simd_callee__ inline void SinForAiryAi(
    Reg::RegTensor<T>& dstReg, Reg::RegTensor<T>& srcReg, Reg::RegTensor<T>& x,
    Reg::RegTensor<T>& round, Reg::RegTensor<T>& kpi, Reg::MaskReg mask)
{
    ...
    CommonCalInTri<T>(dstReg, x, round, kpi, mask);
}

template<typename T>
__simd_callee__ inline void CosForAiryAi(
    Reg::RegTensor<T>& dstReg, Reg::RegTensor<T>& srcReg, Reg::RegTensor<T>& x,
    Reg::RegTensor<T>& round, Reg::RegTensor<T>& kpi, Reg::MaskReg mask)
{
    Reg::Muls(round, srcReg, PI_FOR_X_TODIV, mask);
    ...
    Reg::RegTensor<T>& tempRes, Reg::MaskReg& mask)
{
    ...
    Reg::RegTensor<T> triX, triRound, triKpi;
    ...
    SinForAiryAi<T>(sinT, t, triX, triRound, triKpi, mask);
    Reg::Mul(sinT, sinT, afn, mask);

    Reg::Div(agn, agn, agd, mask);
    Reg::Mul(agn, z, agn, mask);

    CosForAiryAi<T>(cosT, t, triX, triRound, triKpi, mask);
