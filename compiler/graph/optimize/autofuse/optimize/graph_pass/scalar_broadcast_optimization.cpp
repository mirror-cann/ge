...
static bool IsVfNodeScalarUnsupported(const ge::AscNodePtr &next_node, const ge::AscNodePtr &first_brc_node) {
  // 判断节点是否支持VF
  ...
  const auto &source_node = std::dynamic_pointer_cast<ge::AscNode>(first_brc_node->GetInDataNodes().at(0UL));
  ...
  // 源节点不是Scalar节点，跳过优化
  if (!IsOps<ge::ascir_op::Scalar>(source_node)) {
...
Status ScalarBroadcastOptimizationPass::RunPass(ge::AscGraph &graph) {
  for (const auto &node : graph.GetAllNodes()) {
    ...
    const auto &first_brc_node = std::dynamic_pointer_cast<ge::AscNode>(node);
