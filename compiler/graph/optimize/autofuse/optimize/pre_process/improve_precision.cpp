...
namespace {
using ge::AscGraph;
using ge::AscGraphUtils;
using ge::AscNodeAttr;
using ge::AscTensorAttr;
...
bool CheckCastDtype(DataType input_dtype, DataType output_dtype) {
  ...
  return ScheduleUtils::CallAscirInferDataType<ge::ascir_op::Cast>(input_dtypes, expect_output_dtypes) == ge::SUCCESS;
}
...
const std::unordered_map<std::string, std::string> kBlackList1 = {
  {ge::ascir_op::Data::Type, ge::ascir_op::Data::Type},
  {ge::ascir_op::Load::Type, ge::ascir_op::Load::Type},
  {ge::ascir_op::Scalar::Type, ge::ascir_op::Scalar::Type},
  {ge::ascir_op::Store::Type, ge::ascir_op::Store::Type},
  {ge::ascir_op::Output::Type, ge::ascir_op::Output::Type},
  {ge::ascir_op::Broadcast::Type, ge::ascir_op::Broadcast::Type},
  {ge::ascir_op::Transpose::Type, ge::ascir_op::Transpose::Type},
  {ge::ascir_op::Concat::Type, ge::ascir_op::Concat::Type},
  {ge::ascir_op::Gather::Type, ge::ascir_op::Gather::Type},
  {"Slice", "Slice"}
};
...
Status CheckNodeDtype(const NodePtr &node) {
  ...
  if (ge::ascir::CommonInferDtype(node->GetType(), input_dtypes, expect_output_dtypes, npu_arch) != ge::SUCCESS) {
    GELOGE(ge::FAILED,
...
const std::unordered_map<std::string, std::string> kTypeToGroup = {
  {ge::ascir_op::Cast::Type, ge::ascir_op::Cast::Type},
  {ge::ascir_op::Load::Type, ge::ascir_op::Load::Type},
  {ge::ascir_op::Gather::Type, ge::ascir_op::Gather::Type},
  {ge::ascir_op::Scalar::Type, ge::ascir_op::Scalar::Type},
  {ge::ascir_op::Store::Type, ge::ascir_op::Store::Type}
...
bool ShouldChangeDataType(const NodePtr &node, const std::vector<NodePtr> &peer_in_nodes, DataType peer_output_dtype,
  ...
  (void) node;
  if (IsNodeTypeInPeerInNodes(ge::ascir_op::Store::Type, peer_in_nodes)) {
    return false;
  }
...
bool IsFloatToUltraLowNeedInsertCast(const NodePtr &peer_out_node, DataType peer_output_dtype, DataType output_dtype) {
  ...
  if ((type == ge::ascir_op::Load::Type || type == ge::ascir_op::Gather::Type ||
       type == ge::ascir_op::Cast::Type) &&
...
NodePtr BuildCastNode(AscGraph &asc_graph, const NodePtr &ref_node) {
  ...
  auto op = std::make_shared<ge::Operator>(OpDescUtils::CreateOperatorFromOpDesc(cast_op_desc));
...
Status IsNeedInsertCastAfterLoad(const NodePtr &node, bool &is_need_insert_cast) {
  ...
  std::vector<NodePtr> peer_in_nodes;
  GE_ASSERT_SUCCESS(GetPeerInNodes(node, peer_in_nodes, 0));
  if (IsNodeTypeInPeerInNodes(ge::ascir_op::Cast::Type, peer_in_nodes) ||
      IsNodeTypeInPeerInNodes(ge::ascir_op::Store::Type, peer_in_nodes) ||
...
Status IsNeedInsertCastBeforeOther(const NodePtr &other_node, bool &need_insert, std::vector<int32_t> &input_idxs) {
  ...
  for (auto idx = 0U; idx < peer_out_nodes.size(); idx++) {
    ...
    GE_ASSERT_SUCCESS(GetOutputTensorDesc(peer_out_node, peer_output_tensor_desc));
    const auto &type = peer_out_node->GetType();
    if (type == ge::ascir_op::Cast::Type || type == ge::ascir_op::Load::Type || type == ge::ascir_op::Gather::Type) {
...
bool ShouldSkipGraph(GraphPropertiesCache &cache, const AscGraph &asc_graph) {
...
Status IsAllNodesInBlacklist(const AscGraph &asc_graph, bool &result) {
  ...
  for (const auto &node : AscGraphUtils::GetComputeGraph(asc_graph)->GetAllNodes()) {
    if (node->GetType() == ge::ascir_op::Output::Type || node->GetType() == ge::ascir_op::Data::Type) {
      continue;
    }
    if (has_all) {
...
Status ProcessNodeGroups(AscGraph &asc_graph, TypeToNodesMap &type_to_nodes) {
  for (const auto &node : type_to_nodes[ge::ascir_op::Cast::Type]) {
    GE_ASSERT_SUCCESS(CastNodeProc(asc_graph, node));
  }
  GE_ASSERT_SUCCESS(ProcessLoadGatherNodes(asc_graph, type_to_nodes[ge::ascir_op::Load::Type]));
  GE_ASSERT_SUCCESS(ProcessLoadGatherNodes(asc_graph, type_to_nodes[ge::ascir_op::Gather::Type]));
  for (const auto &node : type_to_nodes[ge::ascir_op::Scalar::Type]) {
    GE_ASSERT_SUCCESS(FromDtypeToOtherDtype(node, DT_BF16, DT_FLOAT));
    GE_ASSERT_SUCCESS(FromDtypeToOtherDtype(node, DT_FLOAT16, DT_FLOAT));
  }
  GE_ASSERT_SUCCESS(ProcessOtherComputeNodes(asc_graph, type_to_nodes["Other"]));
  GE_ASSERT_SUCCESS(ProcessStoreNodes(asc_graph, type_to_nodes[ge::ascir_op::Store::Type]));
  return ge::SUCCESS;
}
} // namespace
...
ge::Status ImprovePrecisionForAscGraph(AscGraph &asc_graph) {
  ResetUniqueNumber();
  GraphPropertiesCache cache(asc_graph);
