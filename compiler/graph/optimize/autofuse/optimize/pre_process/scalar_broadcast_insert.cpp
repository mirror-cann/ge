...
namespace {
bool IsSkipDownstreamType(const ge::NodePtr &node) {
  const auto &type = node->GetType();
  return type == ge::ascir_op::Broadcast::Type || type == ge::ascir_op::Data::Type ||
         type == ge::ascir_op::Output::Type;
}

ge::AscNodePtr BuildBroadcastNode(ge::AscGraph &asc_graph, const ge::AscNodePtr &scalar_node,
                                   const ge::AscNodePtr &ref_node) {
...
Status InsertBroadcastAfterScalar(ge::AscGraph &asc_graph, const ge::AscNodePtr &scalar_node, bool &inserted) {
  ...
  std::vector<ge::InDataAnchorPtr> compute_anchors;
  ...
  auto ref_node = std::dynamic_pointer_cast<ge::AscNode>(compute_anchors[0]->GetOwnerNode());
  ...
  for (const auto &dst: compute_anchors) {
    GE_ASSERT_GRAPH_SUCCESS(ge::GraphUtils::RemoveEdge(out_anchor, dst));
    GE_ASSERT_GRAPH_SUCCESS(ge::GraphUtils::AddEdge(b_out, dst));
  }
  GE_ASSERT_GRAPH_SUCCESS(ge::GraphUtils::AddEdge(out_anchor, b_node->GetInDataAnchor(0)));
...
ge::Status InsertBroadcastAfterScalarForAscGraph(ge::AscGraph &asc_graph) {
  std::vector<ge::AscNodePtr> scalar_nodes;
  for (const auto &node: asc_graph.GetAllNodes()) {
    if (node->GetType() == ge::ascir_op::Scalar::Type) {
  ...
  if (inserted) {
    GE_ASSERT_GRAPH_SUCCESS(ge::AscGraphUtils::GetComputeGraph(asc_graph)->TopologicalSorting());
  }
  return ge::SUCCESS;
}
} // namespace optimize::pre_process
