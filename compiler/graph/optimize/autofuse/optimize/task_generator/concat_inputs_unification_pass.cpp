...
Status ConcatInputUnificationPass::RunOneGraph(ascir::ImplGraph &graph) {
  ...
  for (const auto &node : graph.GetAllNodes()) {
    if (ge::ops::IsOps<ge::ascir_op::Concat>(node)) {
      ...
      (void)ge::AttrUtils::SetBool(node->GetOpDesc(), kAttrNameNoReuseInputs, true);
...
ge::Status ConcatInputUnificationPass::CollectSharedInputs(const ge::AscNodePtr &concat_node,
  ...
  for (auto &[src_anchor, input_indices] : src_anchor_to_input_indices) {
    if (input_indices.size() > 1UL) {
      // 存在多引用
      auto *src_node = dynamic_cast<const ge::AscNode *>(src_anchor->GetOwnerNodeBarePtr());
      GE_ASSERT_NOTNULL(src_node);
      // Load类型的会单独计算
      if (src_node->attr.api.compute_type != ge::ComputeType::kComputeLoad) {
        GELOGD("src node = %s, output has multiple ref to concat, input indices = %s", src_node->GetName().c_str(),
               ge::ToString(input_indices).c_str());
