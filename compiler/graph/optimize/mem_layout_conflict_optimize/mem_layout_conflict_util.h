...
struct MemLayoutConflictUtil {
  ...
  static bool IsContinuousInputThroughRefNode(InDataAnchor *const in_data_anchor, const bool no_padding,
                                              InDataAnchor *&continuous_node_in_anchor);
  static bool IsContinuousOutputThroughRefNode(OutDataAnchor *const out_data_anchor, const bool no_padding,
                                               OutDataAnchor *&continuous_node_out_anchor);
  ...
  static bool IsRefFromVar(const OutDataAnchorPtr &out_anchor, NodePtr &src_node,
                           const SymbolToAnchors &symbol_to_anchors, const AnchorToSymbol &anchor_to_symbol);
  static void MarkInTypes(const NodePtr &node, const ComputeGraphPtr &graph, const std::unique_ptr<Checker> &checker);
  static void MarkOutTypes(const NodePtr &node, const ComputeGraphPtr &graph, const SymbolToAnchors &symbol_to_anchors,
                           const AnchorToSymbol &anchor_to_symbol, const std::unique_ptr<Checker> &checker);
  static Status FindConflictNodes(const NodeIndexIOVector &all_nodes, AnchorSet &result, const GraphInfo &graph_info,
                                  const std::unique_ptr<Checker> &checker);
  static Status IsGraphExistMemConflictSymbol(const ComputeGraphPtr &graph, const AnchorToSymbol &anchor_to_symbol,
                                              const SymbolToAnchors &symbol_to_anchors, bool &has_conflict);
  ...
  static Status IsWhileNeedInsertIdentityAtOutput(const ComputeGraphPtr &while_body, bool &is_need_insert);
  ...
  static Status CheckWhileConflict(const NodePtr &ctrl_node, std::vector<InDataAnchorPtr> &conflit_anchors);
  static bool IsCtrlNodeSubgraphExistMemConflictSymbol(const ComputeGraphPtr &graph);
  static void ConstructSingleNodeSymbolTable(const std::string &input_symbol, const std::string &output_symbol,
