...
Status PermutePass::Run(ComputeGraphPtr graph) {
  ...
  for (NodePtr &node : graph->GetDirectNode()) {
    ...
    GE_IF_BOOL_EXEC(
        ...
        /// Input format 5D means NHWC in 4D way. So if input origin format is NCHW and
        /// permute parameter list is [0,3,1,2], this permute can be optimised.
        GE_IF_BOOL_EXEC(
            GetLocalOmgContext().format != DOMI_TENSOR_ND,
            // Get input origin format
            for (NodePtr &n : graph->GetDirectNode()) {
              GE_CHECK_NOTNULL(n->GetOpDesc());
              GE_IF_BOOL_EXEC(
                  n->GetOpDesc()->GetType() == PERMUTE, std::queue<NodePtr> q_node; q_node.push(n);
                  bool jump_out = false; while (!q_node.empty()) {
                    ...
                    for (auto &inNode : n_temp->GetInDataNodes()) {
                      ...
                      GE_IF_BOOL_EXEC(
                          AttrUtils::GetInt(inNode->GetOpDesc(), ATTR_NAME_FORMAT, cur_format),
            ...
            int64_t conv_format = 0; GE_IF_BOOL_EXEC(
                ...
                GE_IF_BOOL_EXEC((node->GetOutDataNodesSize() > 0 && node->GetOutDataNodes().at(0) != nullptr &&
  ...
  GE_IF_BOOL_EXEC(
      isolate_nodes.size() != 0, for (auto &node : isolate_nodes) {
