...
Status RefIdentityDeleteOpPass::Run(ComputeGraphPtr graph) {
  ...
  for (auto &node : graph->GetDirectNode()) {
    ...
    CHECK_FALSE_EXEC(
        GetRefNode(node, input_index) != nullptr,
        REPORT_INNER_ERR_MSG("E19999", "Get Ref node of node:%s(%s) failed", node->GetName().c_str(),
                             node->GetType().c_str());
        GELOGE(FAILED, "[Get][RefNode] of node:%s(%s) failed", node->GetName().c_str(), node->GetType().c_str());
        return FAILED);
    CHECK_FALSE_EXEC(
        DealNoOutputRef(node, graph) == SUCCESS,
        GELOGE(FAILED, "[Deal][NoOutputRef] for node:%s failed, index:%d", node->GetName().c_str(), input_index);
        return FAILED);
  }
  return SUCCESS;
}
