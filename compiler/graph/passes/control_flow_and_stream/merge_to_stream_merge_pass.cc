...
Status MergeToStreamMergePass::Run(ComputeGraphPtr graph) {
  ...
  for (const auto &node : graph->GetDirectNode()) {
    std::string type;
    GE_CHK_STATUS_RET(GetOriginalType(node, type), "[Get][OriginalType] of node in graph:%s failed.",
                      graph->GetName().c_str());
    ...
    if (merge_op_desc->HasAttr(ATTR_INSERT_BY_MBATCH)) {
      ...
      if (status != ge::SUCCESS) {
        REPORT_INNER_ERR_MSG("E19999", "Set stream_label:%s to op:%s(%s) failed", node->GetName().c_str(),
      ...
      GE_CHK_STATUS_RET(ReplaceMergeNode(graph, node), "[Replace][MergeNode] %s in graph:%s failed.",
...
Status MergeToStreamMergePass::AddActiveNodes(const ComputeGraphPtr &graph, const NodePtr &node) const {
  GE_CHK_BOOL_EXEC(node != nullptr, REPORT_INNER_ERR_MSG("E19999", "Param node is nullptr, check invalid");
  ...
  for (const InDataAnchorPtr &in_data_anchor : node->GetAllInDataAnchors()) {
    ...
    GE_CHK_BOOL_EXEC(active_node != nullptr, return FAILED, "[Create][StreamActiveNode] failed, in_node:%s.",
                     in_node->GetName().c_str());
    GE_CHK_STATUS(GraphUtils::AddEdge(active_node->GetOutControlAnchor(), node->GetInControlAnchor()),
                  "[Add][CtrlEdge] between %s and %s failed.", active_node->GetName().c_str(), node->GetName().c_str());
...
NodePtr MergeToStreamMergePass::CreateActiveNode(const ComputeGraphPtr &graph, const NodePtr &node) const {
  ...
  GE_CHK_BOOL_EXEC(active_node != nullptr,
                   ...
                   return nullptr, "[Add][Node] %s(%s) to graph:%s failed", op_desc->GetName().c_str(),
                          op_desc->GetType().c_str(), graph->GetName().c_str());
  GE_IF_BOOL_EXEC(
      GraphUtils::AddEdge(node->GetOutControlAnchor(), active_node->GetInControlAnchor()) != SUCCESS,
      REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed", node->GetName().c_str(),
                           node->GetType().c_str(), active_node->GetName().c_str(), active_node->GetType().c_str());
      GELOGE(INTERNAL_ERROR, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed", node->GetName().c_str(),
             node->GetType().c_str(), active_node->GetName().c_str(), active_node->GetType().c_str());
      return nullptr);
