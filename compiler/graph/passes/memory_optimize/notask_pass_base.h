...
protected:
  ...
  // 子类可选实现：格式校验，默认不过滤
  virtual bool CheckFormat(const ge::OpDescPtr &op_desc) const { return true; }
  // 子类可选实现：返回算子名用于日志
  virtual std::string GetOpLabel() const { return "notask"; }
  ...
  bool CheckDimAlignment(const ge::OpDescPtr &op_desc, const gert::Shape &align_shape,
  ...
  // 单个输入的轴维度校验（公共逻辑：构造src_shape → GetAlignedShape → GetTransferDims → CheckRealDim → CheckDimAlignment）
