...
class HcclOfflineOptionBuilder {
 public:
  ...
  Status Initialize(const std::string &soc_version, const std::string &logic_topo_config,
                    const std::string &hccl_sub_comm_config);
  void Finalize();
  const std::string &GetLogicRankTable() const {
    return logic_rank_table_;
  }
  const std::string &GetHcclCommConfig() const {
    return hccl_comm_config_;
  }
  const std::string &GetHcomGrouplist() const {
    return hcomm_grouplist_;
  }
