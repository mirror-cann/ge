...
bool HandleManager::LaunchDynamicLib() {
  ...
  if (!res) {
    ...
    if (!res) {
      std::map<std::string, std::string> pythonPathMap = {{"value", TeConfigInfo::Instance().GetEnvPath()},
                                                          {"env", "PATH"},
                                                          {"situation", "executing the cmd python3 -V and python -V"},
  ...
  if (strcmp(PY.c_str(), buffer) != 0 || x != X_STD || y < Y_STD) {
    std::map<std::string, std::string> pythonVersionMap = {{"currentVersion", line}, {"supportVersion", "Python 3.7"}};
    TeErrMessageReport(EM_PYTHON_VERSION_INVALID_ERROR, pythonVersionMap);
    TE_ERRLOG(
        "The current Python version is [%d.%d.%d], but only Python 3 is supported and version should no less than 3.7.",
        x, y, z);
    pclose(fp);
    return false;
  }
  ...
  if (!LoadDynLibFromPyCfg(pythonLibPath, handle) || handle == nullptr) {
    ...
    if (libHandle == nullptr) {
      std::map<std::string, std::string> pythonPathMap = {
          {"value", TeConfigInfo::Instance().GetEnvPath()},
          {"env", "PATH"},
