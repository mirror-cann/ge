    ...
    if add_sign == "true":
        ...
        with open(inicfg, "w+", encoding="utf-8") as read_cfg:
            ...
            for infile, conf_item in list(item_size_set.items()):
                ...
                if "cms" in conf_item.type.split("/") and conf_item.tag != "" and os.path.isfile(inputfile):
                    cms_flag = True
                    read_cfg.write(
    ...
    for file in sign_dict["cms"]:
        ...
        if os.path.isfile(file_with_path):
            ...
            if not os.path.isfile(file_sign_des):
                COMM_LOG.cilog_error(THIS_FILE_NAME, "copy %s --> %s fail", file_with_path, file_sign_des)
                return -1
        else:
...
def add_bios_esbc_header(root_dir, item_size_set, sign_file_dir):
    ...
    返回：-1:失败，0：成功
    """
    bios_esbc_header_tool_path = os.path.join(root_dir, "scripts", "signtool", "esbc_header")
    # 检查加头工具目录是否存在
...
def main(argv=None):
    ...
    ret_code = add_bios_header(item_size_set, sign_file_dir, bios_tool_path, sgn_tool_path, root_dir, add_sign)
    return ret_code
