...
TEST(PythonAdapterManagerUT, parse_result_case0) {
  ...
  bool ret = PythonAdapterManager::ParseResult(nullptr, 0, jsonFilePath, pattern, coreType, compileInfo, compileInfoKey,
                                               fusionCheckResCode);
  EXPECT_EQ(ret, false);
  std::string opResCompile = "{\"pattern\":\"Opaque\",\"core_type\":\"AiCore\"}";
  ret = PythonAdapterManager::ParseResult(opResCompile.c_str(), 0, jsonFilePath, pattern, coreType, compileInfo,
                                          compileInfoKey, fusionCheckResCode);
  EXPECT_EQ(ret, true);
...
TEST(PythonAdapterManagerUT, parse_result_case1) {
  ...
  std::string compileInfoKey;
  int fusionCheckResCode = 0;
  bool ret = PythonAdapterManager::ParseResult(nullptr, 1, jsonFilePath, pattern, coreType, compileInfo, compileInfoKey,
                                               fusionCheckResCode);
  EXPECT_EQ(ret, false);
  std::string opResCompile = "{\"pattern\":\"Opaque\",\"fusion_check_result\":1}";
  ret = PythonAdapterManager::ParseResult(opResCompile.c_str(), 1, jsonFilePath, pattern, coreType, compileInfo,
                                          compileInfoKey, fusionCheckResCode);
  EXPECT_EQ(ret, true);
