...
ge::graphStatus FakeGetOption(ge::GEThreadLocalContext *that, const std::string &optionExec,
...
ge::graphStatus OffloadGetOption1(ge::GEThreadLocalContext *that, const std::string &optionExec,
                                  std::string &dumpDebugValue) {
...
std:
  ...
  memcpy(reinterpret_cast<int64_t *>(reinterpret_cast<char *>(privateDefPtr) + sizeof(HCCL_KERNEL_INFO_PRIVATE_DEF)),
  ...
  MOCKER(hrtMemAsyncCopy).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));

  MOCKER(hrtMemcpyAddrAsync).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_SetUnkownWorkSpace) {
  ...
  MOCKER(HcomSupportDeterministicOptim).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_GetHcomOpMemSize) {
  ...
  ret = hcomOpsKernelInfoStore_.GetHcomOpMemSize(shapeType, sCollectiveType, hcomComm, sGroup, dataType, count,
                                                 inputAddrSize, outputAddrSize);
  EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(HcomKernelInfoTest, ut_GetCommCCLBuf) {
  ...
  MOCKER(HcomGetInCCLbuffer).stubs().will(returnValue(HCCL_SUCCESS));
  MOCKER(HcomGetOutCCLbuffer).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_hcom_alltoallv_loadtask) {
  ...
  CHK_BBIT_RET(hrtMemSyncCopy(inputDevMem.ptr(), memSize, inputHostMem.ptr(), memSize,
  ...
  MOCKER(aclrtMemcpyAsync).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_hcom_offline_build_init_hcom) {
  ...
  MOCKER(HcceGetandClearOverFlowTasks).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_opKernelLoop) {
  ...
  MOCKER(HcomGetRankSize).stubs().will(invoke(stub_HcclCommGraphGetRankSize));

  MOCKER(HcomGetRankId).stubs().will(invoke(stub_HcclCommGraphGetRankId));

  MOCKER(HcomGetRankId).stubs().will(invoke(stub_HcomGetRankId));

  DevType type = DevType::DEV_TYPE_910B;
  MOCKER(HcomGetDeviceType).stubs().will(returnValue(type));
  ...
  MOCKER_CPP(&HcomOpsKernelInfoStore::CheckHcomOpMemSize).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
  ...
  MOCKER(HcomAllGather).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));

  MOCKER(HcomReduceScatter).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
...
ge::graphStatus RoleTableGetOption(ge::GEThreadLocalContext *that, const std::string &optionExec,
                                   std::string &dumpDebugValue) {
  dumpDebugValue = "10";
  return ge::GRAPH_SUCCESS;
}

ge::graphStatus EsMaxRemoteOpNumGetOption1(ge::GEThreadLocalContext *that, const std::string &optionExec,
                                           std::string &dumpDebugValue) {
  dumpDebugValue = "0.0";
  return ge::GRAPH_SUCCESS;
}
...
TEST_F(HcomKernelInfoTest, ut_HcomAicpuStreamRegister) {
  ...
  MOCKER_CPP(&HcomOpsKernelInfoStore::GetCommFromTaskInfo)
      .stubs()
      .with(mockcpp::any(), outBound(commStub))
      .will(returnValue(HCCL_SUCCESS));
  MOCKER_CPP(&HcomOpsKernelInfoStore::GetGroupFromTaskInfo)
      .stubs()
      .with(mockcpp::any(), outBound(group1))
      .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelInfoTest, ut_CleanInterMemoryV2_When_MallocFail_Expect_ReturnHCCL_E_INTERNAL) {
  ...
  MOCKER(hrtMalloc).stubs().with(mockcpp::any(), mockcpp::any(), mockcpp::any()).will(returnValue(HCCL_E_INTERNAL));
...
TEST_F(HcomKernelInfoTest, ut_CleanInterMemoryV2_When_MemSyncCopyFail_Expect_ReturnHCCL_E_INTERNAL) {
  ...
  MOCKER(hrtMalloc).stubs().with(mockcpp::any(), mockcpp::any(), mockcpp::any()).will(returnValue(HCCL_SUCCESS));

  MOCKER(hrtMemSyncCopy).stubs().with(mockcpp::any()).will(returnValue(HCCL_E_INTERNAL));
...
TEST_F(HcomKernelInfoTest, ut_CleanInterMemoryV2_When_MemAsyncCopyFail_Expect_ReturnHCCL_E_INTERNAL) {
  ...
  MOCKER(hrtMemAsyncCopy).stubs().with(mockcpp::any()).will(returnValue(HCCL_E_INTERNAL));
...
TEST_F(HcomKernelInfoTest, ut_GetInputCCLbufPtrAndIndirectInCCLbufPtr_When_Normal_Expect_ReturnlsHCCL_SUCCESS) {
  ...
  hcomKernelInfo.GetInputCCLbufPtrAndIndirectInCCLbufPtr(hcomComm, sGroup, inputAddr, inputAddrSize, outputAddr,
                                                         OutputAddrSize);
  hcomKernelInfo.GetOutputCCLbufPtrAndIndirectOutCCLbufPtr(hcomComm, sGroup, inputAddr, inputAddrSize, outputAddr,
                                                           OutputAddrSize);
  GlobalMockObject::verify();
}
