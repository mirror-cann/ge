...
static inline float fp16_ieee_to_fp32_value(uint16_t h) {
   ...
   * - Inf and NaN values in the inputs should become Inf and NaN values after conversion to the single-precision
   * number. Therefore, if the biased exponent of the half-precision input was 0x1F (max possible value), the biased
   * exponent of the single-precision output must be 0xFF (max possible value). We do this correction in two steps:
   ...
   *
   * Note that the above operations do not handle denormal inputs (where biased exponent == 0). However, they also do
   * not operate on denormal inputs, and do not produce denormal results.
   */
   ...
   *
   * The last step is to adjust the bias of the constructed single-precision number. When the input half-precision
   * number is zero, the constructed single-precision number has the value of FP32 = 1 * 2**(126 - 127) = 2**(-1) = 0.5
  ...
  /*
   * - Choose either results of conversion of input as a normalized number, or as a denormalized number, depending on
   * the input exponent. The variable two_w contains input exponent in bits 27-31, therefore if its smaller than 2**27,
   * the input is either a denormal number, or zero.
