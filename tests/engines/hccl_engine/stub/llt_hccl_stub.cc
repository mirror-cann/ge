...
HcclResult sal_get_current_time(char *timeStr, u32 len) {
  ...
  int iLen = snprintf_s(timeStr, len, len, "%04d-%02d-%02d %02d:%0d:%02d.%06u", now->tm_year + TIME_FROM_1900,
...
aclError aclrtUnSubscribeReport(uint64_t threadId, aclrtStream stream) {
  ...
  HCCL_INFO("llt rt UnSubscribe Report success thread[%llu] stream[%p] tmpVectorSize[%d]", threadId, stream,
            tmpVectorSize);
  return ACL_SUCCESS;
}
  ...
  while (!__sync_bool_compare_and_swap(&(notify_shm->record_cnt[ipc_notify->notify_id]), 1, 0)) {
    ...
    if (timeout_cnt <= 0) {
      HCCL_ERROR("wait timeout : record_cnt[%d], device_id[%d], notify_id[%llu]",
                 notify_shm->record_cnt[ipc_notify->notify_id], notify_shm->device_id, ipc_notify->notify_id);
      return ACL_ERROR_RT_PARAM_INVALID;
    }
  }
...
s32 sal_create_name_map(const char *name, rt_name_map_stub_t **name_map) {
  ...
  ret = snprintf_s(mapped_name, SAL_DMEM_UNIQUE_ID_BYTES, SAL_DMEM_UNIQUE_ID_BYTES - 1, "%s%s",
                   SAL_DMEM_UNIQUE_ID_PREFIX, name);
  if (-1 == ret) {
...
rtError_t rtMalloc(void **devPtr, uint64_t size, rtMemType_t type, uint16_t moudleId) {
  ...
  ret = snprintf_s(mem_name, SAL_DMEM_UNIQUE_ID_BYTES, SAL_DMEM_UNIQUE_ID_BYTES - 1, "%s%s", SAL_DMEM_UNIQUE_ID_PREFIX,
                   my_unique_id);
...
aclError aclrtFree(void *devPtr) {
  ...
  for (int i = 0; i < shm_head->relate_ptr_cnt; i++) {
    rt_name_map_stub_t *name_map_ptr =
        (rt_name_map_stub_t *)shm_head->relate_ptr[i];  // name map的地址作为本块共享内存的关联指针
    /* 关闭该段内存的name map */
...
void Adx::AdumpPrintWorkSpace(const void *workSpaceAddr, const size_t dumpWorkSpaceSize, rtStream_t stream,
...
s32 thread_class::stop_thread() {
  ...
  if (threadfd) {
    ...
    // 等待任务主动停止
    for (uiStopCounter = THREAD_STOP_COUNTER; (THREAD_STATE_STOPED != uithreadstate) && uiStopCounter;
         uiStopCounter--) {
        ...
        for (iter = stream_task_list.begin(); iter != stream_task_list.end();) {
          ...
          switch (iter->task_type) {
            case TASK_TYPE_MEMCPY: {
              ...
              rtError_t ret = memcpy_async(iter->stream_para.memcpystruct.dst, iter->stream_para.memcpystruct.src,
                                           iter->stream_para.memcpystruct.count);
              u64 duration = TimestampNanosecond() - ts_start;
              HWTSLog(*iter, ts_start, duration);

              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task MemcpyAsync failed, ret[%d], para:dst[%d] src[%d] count[%d].", ret,
                           iter->stream_para.memcpystruct.dst, iter->stream_para.memcpystruct.src,
                           iter->stream_para.memcpystruct.count);
              }
            ...
            case TASK_TYPE_RECORD: {
              ...
              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task event record failed, ret[%d], para:event[%p],sem[%p].", ret,
                           iter->stream_para.event.event_handler, iter->stream_para.event.sem);
              }
            ...
            case TASK_TYPE_MULTIDEV_RECORD: {
              ...
              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task event multidev record failed, ret[%d], para:event[%p],sem[%p].", ret,
                           iter->stream_para.event.event_handler, iter->stream_para.event.sem);
              }
            ...
            case TASK_TYPE_WAIT: {
              ...
              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task event wait failed, ret[%d], para:event[%p],sem[%p].", ret,
                           iter->stream_para.event.event_handler, iter->stream_para.event.sem);
              }
            ...
            case TASK_TYPE_REDUCE: {
              u64 ts_start = TimestampNanosecond();
              cce::ccStatus_t ret =
                  vector_reduce(iter->stream_para.reducestruct.src1, iter->stream_para.reducestruct.src2,
                                iter->stream_para.reducestruct.count_reduce, iter->stream_para.reducestruct.datatype,
                                iter->stream_para.reducestruct.op, iter->stream_para.reducestruct.dst_reduce);
              u64 duration = TimestampNanosecond() - ts_start;
              HWTSLog(*iter, ts_start, duration);

              if (cce::CC_STATUS_SUCCESS != ret) {
                HCCL_ERROR(
                    "Task vector reduce failed, ret[%d], para:src1[%d] src2[%d] count_reduce[%d] datatype[%d] op[%d] "
                    "dst_reduce[%d].",
                    ret, iter->stream_para.reducestruct.src1, iter->stream_para.reducestruct.src2,
                    iter->stream_para.reducestruct.count_reduce, iter->stream_para.reducestruct.datatype,
                    iter->stream_para.reducestruct.op, iter->stream_para.reducestruct.dst_reduce);
              }
            ...
            case TASK_TYPE_USLEEP: {
              ...
              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task MemcpyAsync failed, ret[%d], para:usec[%d].", ret, iter->stream_para.usec);
              }
            ...
            case TASK_TYPE_RDMA_SEND: {
              ...
              if (RT_ERROR_NONE != ret) {
                HCCL_ERROR("Task RdmaSend Async failed, ret[%d], wqe_index[%d]", ret,
                           iter->stream_para.rdmasend.wqe_index);
              }
...
cce::ccStatus_t stream_class::vector_reduce(const void *src1, const void *src2, uint32_t count,
                                            const cce::ccDataType_t datatype, const cce::ccReduceOp_t op, void *dst) {
  int32_t loop;
...
void *__WorkSpaceMemAllocStub(std::string tag, u64 size) {
  ...
  CHK_PRT_RET(ret, HCCL_ERROR("rt_malloc fail, tag[%s], size[%llu], ret[%d]", tag.c_str(), size, ret), NULL);
  return ptr;
}
...
int ibv_ext_post_send(struct ibv_qp *qp, struct ibv_send_wr *wr, struct ibv_send_wr **bad_wr,
                      struct ibv_post_send_ext_attr *ext_attr, struct ibv_post_send_ext_resp *ext_resp) {
  return 0;
}

int stub_ibv_ext_post_send(struct ibv_qp *qp, struct ibv_send_wr *wr, struct ibv_send_wr **bad_wr,
                           struct ibv_post_send_ext_attr *ext_attr, struct ibv_post_send_ext_resp *ext_resp) {
...
namespace hccl {
const std::unordered_map<std::string, DevType> SOC_VER_CONVERT{{"Ascend310P1", DevType::DEV_TYPE_310P1},
...
aclError aclrtKernelArgsGetPlaceHolderBuffer(aclrtArgsHandle argsHandle, aclrtParamHandle paramHandle, size_t dataSize,
                                             void **bufferAddr) {
  return ACL_SUCCESS;
}
