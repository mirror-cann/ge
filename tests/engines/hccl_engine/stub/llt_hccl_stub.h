...
namespace Adx {
enum class DumpType : int32_t { OPERATOR = 0x01, EXCEPTION = 0x02, ARGS_EXCEPTION = 0x03, OP_OVERFLOW = 0x04 };
bool AdumpIsDumpEnable(DumpType type);
void AdumpPrintWorkSpace(const void *workSpaceAddr, const size_t dumpWorkSpaceSize, rtStream_t stream,
...
enum class QosEngineType : int { AI, HCCL, AICPU, MEMCPYS, CMO };
...
// 互斥锁保护任务队列
#define MSG_LOCK() \
  if (stream_task_lock) (void)sal_mutex_take(stream_task_lock, SAL_MUTEX_FOREVER)

#define MSG_UNLOCK() \
  if (stream_task_lock) (void)sal_mutex_give(stream_task_lock)
...
class stream_class : private thread_class {
 private:
  ...
  rtError_t event_multidev_record(
      rtEvent_t event);  // 实际任务执行体:桩函数rtMultiDevEventRecord实现内容，完成片间event record任务
  rtError_t event_wait(rtEvent_t event);  // 实际任务执行体:桩函数rtStreamWaitEvent实现内容，完成event wait任务
  rtError_t memcpy_async(void *dst, void *src,
                         uint64_t count);  // 实际任务执行体:桩函数rtMemcpyAsync实现内容，完成asynchronous Memcpy任务
  rtError_t notify_record(
      rt_notify_t *notify);  // 实际任务执行体:桩函数rtNotifyRecord实现内容，完成asynchronous NotifyRecord任务
  rtError_t notify_wait(
      rt_notify_t *notify);  // 实际任务执行体:桩函数rtNotifyWait实现内容，完成asynchronous NotifyWait任务
  template <typename T>
  cce::ccStatus_t reduce_op(T src1, T src2, T *dst, const cce::ccReduceOp_t op);
  cce::ccStatus_t vector_reduce(const void *src1, const void *src2, uint32_t count, const cce::ccDataType_t datatype,
                                const cce::ccReduceOp_t op,
                                void *dst);  // 实际任务执行体:桩函数ccVectorReduce实现内容，完成规约操作任务
 ...
 public:
  ...
  static rtError_t rdma_send(u32 wqe_index,
                             void *cnn);  // 实际任务执行体:桩函数rtRDMASend实现内容，完成asynchronous RDMASend任务
