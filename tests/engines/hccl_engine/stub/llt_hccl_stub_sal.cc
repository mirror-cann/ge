...
s32 sal_thread_is_running(sal_thread_t thread) {
  ...
  if (ti) {
    ...
    if (sal_thread_is_running_internal(thread)) {
      ...
      HCCL_WARNING("Bug: thread[%s] is not running, but task info is remain in mem. Clean it now", ti->name.c_str());
      sal_manage.sal_thread_unregist(ti);
      delete ti;
    }
  }
...
void *sal_share_memory_create(const char *uniqueId, u64 memSize) {
  ...
  /* 共享内存管理数据初始化 */
  hcclRet =
      sal_strncpy(shareMemPtr->rootInfo, SAL_SHARE_MEM_UNIQUE_ID_BYTES, uniqueId, (SAL_SHARE_MEM_UNIQUE_ID_BYTES - 1));
...
void sal_share_memory_destroy(void *ptr) {
  ...
  hcclRet =
      sal_strncpy(uniqueId, SAL_SHARE_MEM_UNIQUE_ID_BYTES, shareMemPtr->rootInfo, (SAL_SHARE_MEM_UNIQUE_ID_BYTES - 1));
...
HcclResult SalGetUniqueId(char *salUniqueId) {
  ...
  CHK_PRT_RET(sRet != EOK,
              HCCL_ERROR("In get unique id, mem set failed.errorno[%d], "
                         "params: uniqueId[%p], dest max size[%d], set value[%d], set length[%d]",
                         sRet, salUniqueId, SAL_UNIQUE_ID_BYTES, 0, SAL_UNIQUE_ID_BYTES),
              HCCL_E_MEMORY);
  ...
  s32 hcclRet = snprintf_s(salUniqueId, SAL_UNIQUE_ID_BYTES, SAL_UNIQUE_ID_BYTES - 1, "%08x-%08x-%08x", myPid,
...
HcclResult rt_stream_synchronize(HcclRtStream stream) {
  ...
  CHK_PRT_RET(ret != RT_ERROR_NONE,
              HCCL_ERROR("errNo[0x%016llx] rt stream synchronize fail. return[%d], "
                         "para: rt_stream[%p].",
