...
GraphConstructor &GraphConstructor::AddOpDesc(const string &op_name, const string &op_type, const size_t &inputs_size,
  ...
  if (iter == op_map_.end()) {
    ...
    FE_MAKE_SHARED(ge_op_desc_ptr = std::make_shared<ge::OpDesc>(op_name.c_str(), op_type), ge_op_desc_ptr = nullptr;
                   return *this);
...
GraphConstructor &GraphConstructor::AddOpDesc(const OpImplType &impl_type, const string &pattern, const string &op_name,
                                              const string &op_type, const size_t &inputs_size,
                                              const size_t &outputs_size) {
...
Status GraphConstructor::SetTensorDescInfo(ge::GeTensorDesc &tensor, const ge::Format &original_format,
  ...
  GC_LOGD("Current and original format is %u and %u, original shape is %s", currentformat, original_format,
  ...
  if (currentformat == original_format) {
    ...
    ShapeAndFormat shape_and_format_info = {origin_shape_afer_pad, new_shape, original_format, currentformat,
                                            data_type};
    ...
    if (output_anchor != nullptr) {
      for (auto &peer_in_data_anchor : output_anchor->GetPeerInDataAnchors()) {
        ...
        if (new_node->GetAllOutDataAnchors().size() > out_anchor_index) {
          ...
          FE_LOGE("new node %s does not have enough output anchors.", new_node->GetName().c_str());
          return FAILED;
        }
      }
    }
  ...
  for (auto &dst_node : dst_connection_info_of_all_nodes) {
    ...
    for (auto &src_node : src_connection_info_of_all_nodes) {
      ...
      if (dst_node.op_name == src_node.op_name) {
        FE_LOGE("Cannot create self loop edge for node %s %d->%d", src_node.op_name.c_str(),
      ...
      if ((src_node.starting_tensor_index == -1 && dst_node.starting_tensor_index != -1) ||
        ...
        FE_LOGE("Cannot create control edge for node %s %d->%d", src_node.op_name.c_str(),
                src_node.starting_tensor_index, dst_node.starting_tensor_index);
        continue;
      ...
      if (ge_dst_op_desc_ptr->GetInputsSize() <= tensor_index) {
        FE_LOGE("The tensor index %d of %s is larger than its inputs size %u", tensor_index,
      ...
      if (output_anchor == nullptr) {
        FE_LOGE("Out put anchor %d of src node %s is nullptr", src_node.starting_tensor_index,
                src_node.op_name.c_str());
        return FAILED;
      }

      if (ge::GraphUtils::AddEdge(output_anchor, in_anchor) != ge::GRAPH_SUCCESS) {
        FE_LOGE("[2]:Failed to Add edge from [%s]: %d to [%s] : %d.", src_node.node->GetName().c_str(),
...
string GraphConstructor::GetInputString(const ge::NodePtr &node) {
  ...
  for (const auto &node : node->GetInControlNodes()) {
    input_node_name_string += ("[" + node->GetName() + ":-1], ");
  }
  input_node_name_string += "}";
...
GraphConstructor &GraphConstructor::SetInputs(const DetailedTensor &dst_tensor,
  ...
  if (CheckOriginalFormatValid(dst_tensor.original_format_) != SUCCESS) {
    FE_LOGE("original format %u of dst node %s is invalid", dst_tensor.original_format_, dst_tensor.name_.c_str());
    return *this;
  }
  for (const auto &src_tensor : multiple_src_tensors) {
    if (CheckOriginalFormatValid(src_tensor.original_format_) != SUCCESS) {
      FE_LOGE("original format %u of src node %s is invalid", src_tensor.original_format_, src_tensor.name_.c_str());
      return *this;
    }
  }
...
GraphConstructor &GraphConstructor::SetInput(const string &dst_name, const string &src_name, const ge::Format &format,
  ...
  GC_LOGD("original dims is %s, format is %u for src %s and dst %s",
          StringUtils::IntegerVecToString(original_dims).c_str(), original_format, src_name.c_str(), dst_name.c_str());
...
Status ParseWhenBothUnderlineAndColonExists(const string &name, string &op_type, string &op_real_name,
  ...
  if (StringUtils::IsInteger(string_between_underline_and_colon) || string_between_underline_and_colon.empty()) {
...
Status ParseWhenUnderlineMissing(const string &name, string &op_type, string &op_real_name,
  ...
  if (colon_position == string::npos) {
    ...
    if (IsSignedInteger(tensor_index) || tensor_index.empty()) {
      ...
      FE_LOGE("The string behind last colon (%s) is not integer!", tensor_index.c_str());
      return FAILED;
    }
  }
  return SUCCESS;
...
bool CheckPeerAnchor(const ge::OutDataAnchorPtr &in_anchor1, const ge::OutDataAnchorPtr &in_anchor2) {
  ...
  if (peer_set1.size() != peer_set2.size()) {
    FE_LOGE("peer1 size %d and peer2 size %d are not the same.", peer_set1.size(), peer_set2.size());
    return false;
  }
...
bool CheckPeerAnchor(const ge::InDataAnchorPtr &in_anchor1, const ge::InDataAnchorPtr &in_anchor2) {
  ...
  if (peer1 == nullptr && peer2 == nullptr) {
    ...
    FE_LOGE("peer1 %s and peer2 %s are not the same.", peer1->GetOwnerNode()->GetName().c_str(),
...
bool CheckTensorDesc(const ge::GeTensorDesc &tensor1, const ge::GeTensorDesc &tensor2) {
  ...
  if (ret == false) {
    FE_LOGE("Format         %u   :  %u", tensor1.GetFormat(), tensor2.GetFormat());
    FE_LOGE("DType          %u   :  %u", tensor1.GetDataType(), tensor2.GetDataType());

    FE_LOGE("OriginalFormat %u   :  %u", tensor1.GetOriginFormat(), tensor2.GetOriginFormat());
    FE_LOGE("OriginalDtype  %u   :  %u", tensor1.GetOriginDataType(), tensor2.GetOriginDataType());
    ...
    string original_shape1 = StringUtils::IntegerVecToString(tensor1.GetOriginShape().GetDims());
    string original_shape2 = StringUtils::IntegerVecToString(tensor2.GetOriginShape().GetDims());
    FE_LOGE("Shape          %s   :  %s", shape1.c_str(), shape2.c_str());
    FE_LOGE("OriginalShape  %s   :  %s", original_shape1.c_str(), original_shape2.c_str());
  }
  return ret;
}
bool GraphConstructor::CompareNode(const ge::NodePtr &node1, const ge::NodePtr &node2) {
  ...
  if (op_desc1->GetInputsSize() != op_desc2->GetInputsSize()) {
    FE_LOGE("Input size of node[%s] %d and [%s] %d are not the same.", node1->GetName().c_str(),
            op_desc1->GetInputsSize(), node2->GetName().c_str(), op_desc2->GetInputsSize());
    return false;
  }
  if (op_desc1->GetOutputsSize() != op_desc2->GetOutputsSize()) {
    FE_LOGE("Output size of node[%s] %d and [%s] %d are not the same.", node1->GetName().c_str(),
  ...
  for (size_t tensor_index = 0; tensor_index < input_size; tensor_index++) {
    if (!CheckTensorDesc(op_desc1->GetInputDesc(tensor_index), op_desc2->GetInputDesc(tensor_index))) {
      FE_LOGE("Input tensor %u of node %s[%s] and %s[%s] are not the same.", tensor_index, node1->GetName().c_str(),
    ...
    if (!CheckPeerAnchor(node1->GetInDataAnchor(tensor_index), node2->GetInDataAnchor(tensor_index))) {
      FE_LOGE("Peer node of input %u of node %s and %s are not the same.", tensor_index, node1->GetName().c_str(),
              node2->GetName().c_str());
      return false;
    }
  }
  ...
  for (size_t tensor_index = 0; tensor_index < output_size; tensor_index++) {
    if (!CheckTensorDesc(op_desc1->GetOutputDesc(tensor_index), op_desc2->GetOutputDesc(tensor_index))) {
      FE_LOGE("Output tensor %u of node %s and %s are not the same.", tensor_index, node1->GetName().c_str(),
              node2->GetName().c_str());
      return false;
    }
    if (!CheckPeerAnchor(node1->GetOutDataAnchor(tensor_index), node2->GetOutDataAnchor(tensor_index))) {
      FE_LOGE("Peer node of output %u of node %s and %s are not the same.", tensor_index, node1->GetName().c_str(),
              node2->GetName().c_str());
      return false;
    }
  }
...
bool GraphConstructor::CompareGraph(const ComputeGraphPtr &graph1, const ComputeGraphPtr &graph2) {
  ...
  if (node_set1.size() != node_set2.size()) {
    FE_LOGE("nodeSet1.size[%d] and node_set2.size[%d] are not the same", node_set1.size(), node_set2.size());
    return false;
  }
  ...
  for (auto &node1 : node_set1) {
    ...
    if (iter == node_map.end()) {
      FE_LOGE("node[%s] of node_set1 not find in node_set2", node1->GetName().c_str());
      return false;
    }
    ge::NodePtr node2 = iter->second;
    if (!CompareNode(node1, node2)) {
      FE_LOGE("Node %s and %s are not the same", node1->GetName().c_str(), node2->GetName().c_str());
      return false;
    }
  }
  return true;
...
GraphConstructor &GraphConstructor::SetFeImPlType(OpImplType impl_type) {
  ...
  ge::AttrUtils::SetInt(opdesc, ge::ATTR_NAME_IMPLY_TYPE, static_cast<int>(iter->second));
  return *this;
}
...
Status GraphConstructor::AddFunctionNodeInputDesc(ge::OpDescPtr fus_op,
...
Status GraphConstructor::AddFunctionNodeOutputDesc(ge::OpDescPtr fus_op,
...
Status GraphConstructor::CreateFunctionOpSubGraph(ge::NodePtr &function_node, std::vector<ge::NodePtr> &node_vec,
  ...
  for (auto &node : node_vec) {
    ...
    FE_LOGD("Begin re-establish connection of node[name:%s]. out data size is %d.", node->GetName().c_str(),
...
Status GraphConstructor::TransSingleSubGraph(ge::ComputeGraph &graph, std::vector<ge::NodePtr> &node_vec) {
  ...
  if (graph_comm_ptr_->GetFusionNodeCtrlEdgeList(node_vec, node_set, input_ctrl_edge_list, output_ctrl_edge_list) !=
      SUCCESS) {
  ...
  if (graph_comm_ptr_->MergeFusionNodeEdgeList(function_node, node_vec, input_edge_list, output_edge_list) != SUCCESS) {
