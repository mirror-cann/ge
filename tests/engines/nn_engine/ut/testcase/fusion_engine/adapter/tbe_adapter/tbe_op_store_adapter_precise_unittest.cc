...
class UTESTTbeOpStoreAdapterPrecise : public testing::Test {
 protected:
  void SetUp() {
...
TEST_F(UTESTTbeOpStoreAdapterPrecise, SerialPreCompileOp_PreBuildFailed_Coverage232) {
  ...
  AttrUtils::SetStr(op_desc, ATTR_NAME_UNREGST_OPPATH, "test_path");
  NodePtr node = graph->AddNode(op_desc);

  vector<PreCompileNodePara> compile_para_vec;
  PreCompileNodePara comp_para;
  comp_para.node = node.get();
  comp_para.op_dsl_file_path = "test_dsl_path";
  comp_para.session_graph_id = "test_session";
...
TEST_F(UTESTTbeOpStoreAdapterPrecise, ParallelPreCompileOp_PreBuildFailed_Coverage628) {
  TbeOpStoreAdapterPtr tbe_adapter_ptr = std::make_shared<TbeOpStoreAdapter>(AI_CORE_NAME);
  tbe_adapter_ptr->Initialize(std::map<std::string, std::string>());
...
TEST_F(UTESTTbeOpStoreAdapterPrecise, SetTaskFusionTask_Failed_Coverage670) {
  TbeOpStoreAdapterPtr tbe_adapter_ptr = std::make_shared<TbeOpStoreAdapter>(AI_CORE_NAME);
  tbe_adapter_ptr->Initialize(std::map<std::string, std::string>());
