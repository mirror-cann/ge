...
TEST_F(UTESTTbeOpStoreAdapterCoverage, SetTaskToTeFusion_TeFusionFailed) {
  ...
  tbe_adapter_ptr->TeFusion = [](vector<ge::Node *> &, ge::OpDescPtr, vector<ge::NodePtr> &, uint64_t, uint64_t,
                                 const string &) { return te::OP_BUILD_FAIL; };

  Status status =
      tbe_adapter_ptr->SetTaskToTeFusion(task_para, {}, nodes, "", CompileStrategy::COMPILE_STRATEGY_NORMAL);
  EXPECT_NE(status, SUCCESS);
}
