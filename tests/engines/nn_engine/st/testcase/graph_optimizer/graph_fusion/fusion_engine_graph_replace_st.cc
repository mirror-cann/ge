...
*/
  vector<FusionRulePatternPtr> CreateFusionRulePatternOne() {
    ...
    // Fusion node
    FusionRuleNodePtr fusion_matmul_node = CreateFusionRuleNode("matMul", {"MatMulCCE"}, {0, 1, 2}, {0}, attributes);
...
*/
  ...
  /*
      ����ƥ�䵽����ͼ
  */
  void CreateMappings(const vector<FusionRulePatternPtr> &patterns, vector<GraphMatchResult> &match_results,
    ...
    for (size_t i = 0; i < patterns.size(); ++i) {
      ...
      for (auto iter = fusion_rule_nodes.begin(); iter != fusion_rule_nodes.end(); ++iter) {
        ...
        const vector<FusionRuleAnchorPtr> &input_anchors = fusion_rule_node->GetInputDataAnchors();
  ...
  vector<FusionRulePatternPtr> CreateFusionRulePatternTwo() {
    ...
    // Fusion node
    FusionRuleNodePtr fusion_node = CreateFusionRuleNode("node4", {"SoftmaxGrad"}, {0, 1}, {0}, attributes);
  ...
  */
  vector<FusionRulePatternPtr> CreateFusionRulePatternThree() {
    ...
    // Fusion node
    FusionRuleNodePtr fusion_node = CreateFusionRuleNode("node4", {"SoftmaxGrad"}, {0, 1}, {0}, attributes);
    FusionRuleNodePtr fusion_node2 = CreateFusionRuleNode("node5", {"Sub"}, {0, 1}, {0}, attributes);
    Link(square_node1, 0, fusion_node, 0);
    Link(data_node2, 0, fusion_node, 1);
