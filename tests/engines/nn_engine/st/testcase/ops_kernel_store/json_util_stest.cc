...
TEST_F(STEST_JSON_UTIL, real_path_succ) {
  std::string filename =
      GetCodeDir() +
      "/tests/engines/nn_engine/st/testcase/ops_kernel_store/fe_config/cce_custom_opinfo/cce_custom_opinfo.json";
  string ret = RealPath(filename);
...
TEST_F(STEST_JSON_UTIL, read_json_file_fail_) {
  std::string filename =
      GetCodeDir() + "/tests/engines/nn_engine/st/testcase/ops_kernel_store/fe_config/opinfo_not_exist.json";
  json j;
  Status ret = ReadJsonFile(filename, j);
