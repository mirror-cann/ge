...
class ArgsParser {
 public:
  ArgsParser(const domi::TaskDef &task_def, OpDescPtr op_desc) : task_def_(task_def), op_desc_(std::move(op_desc)) {}
  virtual ~ArgsParser() = default;
  virtual uint64_t ParseArgsAddr(uint64_t args_addr, bool in, size_t io_index) const = 0;
  virtual bool CheckArgsExtra(uint64_t args_addr) const {
    return true;
  }
...
#define REGISTER_PARSER_CREATOR(type, T)                                                        \
  ...
  inline static ArgsParserFactory::Registerar registerar_ {                                     \
    type, [](const domi::TaskDef &task_def, OpDescPtr op_desc) -> std::unique_ptr<ArgsParser> { \
...
class KernelArgsParser : public ArgsParser {
 public:
  uint64_t ParseArgsAddr(uint64_t args_addr, bool in, size_t io_index) const override {
    size_t offset =
        in ? io_index * sizeof(uint64_t) : (ModelUtils::GetInputSize(op_desc_).size() + io_index) * sizeof(uint64_t);
 ...
 public:
  ...
  uint64_t ParseArgsAddr(uint64_t args_addr, bool in, size_t io_index) const override {
    ...
    for (const auto &iter : format_) {
      if ((iter.addr_type == AddrType::INPUT_INSTANCE && in) || (iter.addr_type == AddrType::OUTPUT_INSTANCE && !in)) {
        return *reinterpret_cast<uint64_t *>(args_addr + offset);
      }
  ...
  bool CheckArgsExtra(uint64_t args_addr) const override {
    ...
    for (const auto &iter : format_) {
      if (iter.addr_type == AddrType::PLACEHOLDER &&
          memcmp(&u8[offset], &magic,
                 iter.ir_idx == static_cast<int32_t>(ArgsFormatWidth::BIT32) ? sizeof(uint32_t) : sizeof(uint64_t)) !=
              0) {
        GELOGE(FAILED, "placeholder args check failed at offset[%zu]", offset);
        return false;
      } else if (iter.addr_type == AddrType::CUSTOM_VALUE &&
                 memcmp(&u8[offset], iter.reserved,
                        iter.ir_idx == static_cast<int32_t>(ArgsFormatWidth::BIT32) ? sizeof(uint32_t)
                                                                                    : sizeof(uint64_t)) != 0) {
