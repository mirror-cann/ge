...
ComputeGraphPtr BuildNeedInsertCastGraph(const vector<int64_t> &shape) {
  ...
  const auto data0 = EsCreateGraphInput(graph.get(), 0);
  EsSetShape(data0, shape.data(), static_cast<int64_t>(shape.size()));
  const auto stridedslice0 = EsStridedSliceD(data0, begin.data(), begin.size(), end.data(), end.size(), strides.data(),
                                             strides.size(), 0, 0, 0, 0, 0);
  const auto stridedslice1 = EsStridedSliceD(data0, begin.data(), begin.size(), end.data(), end.size(), strides.data(),
                                             strides.size(), 0, 0, 0, 0, 0);
  const auto stridedslice2 = EsStridedSliceD(data0, begin.data(), begin.size(), end.data(), end.size(), strides.data(),
                                             strides.size(), 0, 0, 0, 0, 0);
  const auto stridedslice3 = EsStridedSliceD(data0, begin.data(), begin.size(), end.data(), end.size(), strides.data(),
                                             strides.size(), 0, 0, 0, 0, 0);
