...
TEST_F(UtestHccl, set_default_hccl_options_from_offline_builder) {
  ...
  ASSERT_TRUE(
      WriteTextFile(logic_topo_config, R"({"RankTable":[{"rank_id":"1"}],"HcclCommConfig":{"graph_mode":"1"}})"));
  ASSERT_TRUE(WriteTextFile(hccl_sub_comm_config, R"({"group_list":[{"group_name":"g1","group_rank_list":[1,2]}]})"));
