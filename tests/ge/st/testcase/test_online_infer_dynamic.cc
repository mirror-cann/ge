...
static void BuildSampleGraph(ComputeGraphPtr &graph, ComputeGraphPtr &case1, ComputeGraphPtr &case2,
                             uint32_t &mem_offset) {
  ...
  DEF_GRAPH(g0) {
    ...
    CHAIN(NODE("ascend_mbatch_shape_data", data_2)
              ->NODE("ascend_mbatch_shape_mapindex", "MapIndex")
              ->NODE("ascend_mbatch_shape_case", CASE)
              ->NODE(NODE_NAME_NET_OUTPUT, output));
    ...
    CHAIN(NODE("ascend_mbatch_shape_data")
              ->NODE("ascend_mbatch_get_dynamic_dims_node", GETDYNAMICDIMS)
  ...
  DEF_GRAPH(g1) {
    ...
    CHAIN(
        NODE("g1/_arg_0", data_1)->EDGE(0, 0)->NODE("g1/add", ADD)->NODE("g1/output_ascend_mbatch_batch_0", NETOUTPUT));
    CHAIN(NODE("g1/_arg_1", data_2)->EDGE(0, 1)->NODE("g1/add"));

    CHAIN(NODE("g1/_index", data_0)
              ->NODE("g1/switch", label_switch)
              ->CTRL_EDGE()
  ...
  DEF_GRAPH(g2) {
    ...
    auto data_2 = OP_CFG(DATA).Attr(ATTR_NAME_PARENT_NODE_INDEX, 2);
    CHAIN(
        NODE("g2/_arg_0", data_1)->EDGE(0, 0)->NODE("g2/add", ADD)->NODE("g2/output_ascend_mbatch_batch_1", NETOUTPUT));
...
static void TestDynamicBatchSize(const uint32_t model_id) {
  ...
  //{
  //  uint64_t dynamic_input_addr = 0U; uint64_t length = 0U; std::vector<kAippDynamicBatchPara> aipp_batch_para;
  //  kAippDynamicPara aipp_parms; EXPECT_EQ(ge_executor.SetDynamicAippData(model_id, &dynamic_input_addr, length,
  //  aipp_batch_para, aipp_parms), SUCCESS);
  //}
...
static void TestDynamicImageSize(const uint32_t model_id) {
...

  {
    uint64_t dynamic_input_addr = 0U;
    uint64_t length = sizeof(uint64_t);
    uint64_t image_height = 340U;
    uint64_t image_width = 560U;
    EXPECT_EQ(ge_executor.SetDynamicImageSize(model_id, &dynamic_input_addr, length, image_height, image_width),
              SUCCESS);
  }
...
static void TestDynamicDimsSize(const uint32_t model_id) {
  GeExecutor ge_executor;
  {
    std::vector<uint64_t> dynamic_dims{2, 4, 8};
    std::vector<uint64_t> cur_dynamic_dims;
    EXPECT_EQ(ge_executor.GetCurDynamicDims(model_id, dynamic_dims, cur_dynamic_dims), SUCCESS);
  }
...
static Graph BuildGetNextSinkGraphWithSubgraph() {
  DEF_GRAPH(sub_1) {
    const auto data_0 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).Attr(ATTR_NAME_PARENT_NODE_INDEX, 0).Build("sub_1_data_0");
    const auto data_1 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 1).Attr(ATTR_NAME_PARENT_NODE_INDEX, 1).Build("sub_1_data_1");
  ...
  DEF_GRAPH(sub_2) {
    const auto data_0 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).Attr(ATTR_NAME_PARENT_NODE_INDEX, 0).Build("sub_2_data_0");
    const auto data_1 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 1).Attr(ATTR_NAME_PARENT_NODE_INDEX, 1).Build("sub_2_data_1");
...
static Graph CreateHybridGraph() {
  DEF_GRAPH(test_graph) {
    const auto data0 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {-1, 3, 3}).Build("data0");
    const auto data1 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {-1, 3, 3}).Build("data1");
    auto add0 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {}).Build("add0");
...
static Graph CreateDynamicDimsGraph() {
  DEF_GRAPH(test_graph) {
    ...
    const auto data1 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).Build("data1");
...
static Graph CreateDynamicDimsGraphNoData() {
  DEF_GRAPH(test_graph) {
    const auto const0 = OP_CFG(CONSTANT).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).Build("const0");
    const auto const1 = OP_CFG(CONSTANT).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).Build("const1");
    auto add0 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).Build("add0");
    auto mul1 = OP_CFG(MUL).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).InCnt(2).OutCnt(1).Build("mul1");
...
static Graph CreateDynamicDimsGraphWithMoreData() {
  DEF_GRAPH(test_graph) {
    const auto data0 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("cat");
    const auto data1 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 1).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("apple");
    const auto data2 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("bed");
    auto var0 = OP_CFG(VARIABLE).InCnt(1).OutCnt(1).TensorDesc(FORMAT_ND, DT_INT32, {1, 2, 1, 1}).Build("var0");
    std::vector<int32_t> data_value = {100, 100};
    GeTensorDesc data_tensor_desc(GeShape({1, 2, 1, 1}), FORMAT_NCHW, DT_INT32);
    GeTensorPtr tensor =
        make_shared<GeTensor>(data_tensor_desc, (uint8_t *)data_value.data(), data_value.size() * sizeof(int32_t));
    auto const_1 = OP_CFG(CONSTANTOP).Weight(tensor).Build("const_1");
    auto const_2 = OP_CFG(CONSTANTOP).Weight(tensor).Build("const_2");
    auto add0 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("add0");

    auto add1 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("add1");
    auto add2 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("add2");
    auto add3 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("add3");

    auto add4 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {1, 2, 1, 1}).Build("add4");
    auto net_output = OP_CFG(NETOUTPUT).InCnt(1).OutCnt(1).Build("net_output");
...
TEST_F(OnlineInferTest, online_infer_dynamic_execute) {
  ...
  EXPECT_TRUE(AttrUtils::SetListInt(case_desc, "_combined_batch_1", {4}));
  {
    ...
    EXPECT_EQ(OnlineInferDynamic(graph, ge_model, ome_context, &TestDynamicBatchSize),
              SUCCESS);  // ParseInputsDimsForData
  }
  ...
  EXPECT_TRUE(AttrUtils::SetListInt(case_desc, "_combined_batch_1", {1, 2}));
  {
    ...
    EXPECT_EQ(OnlineInferDynamic(graph, ge_model, ome_context, &TestDynamicImageSize),
              SUCCESS);  // ParseInputsDimsForGetNextNoSinkAndData
  }
  ...
  EXPECT_TRUE(AttrUtils::SetListInt(case_desc, "_combined_batch_1", {1, 2}));
  {
    ...
    ome_context.data_nodes.emplace_back(mbatch_batch_node);
    ome_context.getnext_nosink_nodes.emplace_back(mbatch_batch_node);
    EXPECT_EQ(OnlineInferDynamic(graph, ge_model, ome_context, &TestDynamicDimsSize),
              SUCCESS);  // ParseInputsDimsForGetNextNoSinkAndData
  }
  ...
  EXPECT_TRUE(AttrUtils::SetListStr(op_desc, ATTR_NAME_DYNAMIC_OUTPUT_DIMS, dynamic_shape_info));
  {
    ...
    ome_context.getnext_nosink_nodes.emplace_back(mbatch_batch_node);
    EXPECT_EQ(OnlineInferDynamic(graph, ge_model, ome_context, &TestDynamicDimsSize, true),
              SUCCESS);  // ParseInputsDimsForData
  }
...
TEST_F(OnlineInferTest, online_infer_hybrid_mode) {
  ...
  map<AscendString, AscendString> options = {{"ge.compileHybridMode", "1"},
                                             {"ge.inputShape", "data0:-1,3,3;data1:-1,3,3"},
                                             {"ge.dynamicDims", "1,1;10,10"},
...
TEST_F(OnlineInferTest, online_infer_dynamic_dims_graph_with_scalar_input) {
  ...
  DUMP_GRAPH_WHEN("PreRunAfterOptimize1");
  map<AscendString, AscendString> options = {{"ge.inputShape", "data0:-1;data1:-1;data2:"},
...
TEST_F(OnlineInferTest, online_infer_dynamic_dims_graph_three_data) {
  ...
  DUMP_GRAPH_WHEN("PreRunAfterOptimize1");
  // new session & add graph
  map<AscendString, AscendString> options = {{"ge.inputShape", "apple:-1,2,-1,-1;bed:-1,2,1,-1;cat:1,2,-1,-1"},
  ...
  CHECK_GRAPH(PreRunAfterOptimize1) {
    ...
    std::map<std::string, std::vector<int64_t>> expect_shape_map = {
        {"apple_ascend_mbatch_batch_0", {1, 2, 1, 1}}, {"bed_ascend_mbatch_batch_0", {1, 2, 1, 1}},
        {"cat_ascend_mbatch_batch_0", {1, 2, 1, 1}},   {"apple_ascend_mbatch_batch_1", {1, 2, 2, 2}},
        {"bed_ascend_mbatch_batch_1", {1, 2, 1, 1}},   {"cat_ascend_mbatch_batch_1", {1, 2, 1, 1}},
        {"apple_ascend_mbatch_batch_2", {1, 2, 3, 3}}, {"bed_ascend_mbatch_batch_2", {1, 2, 1, 1}},
...
TEST_F(OnlineInferTest, GraphMultiDimsWithMiddleDynamicShape_ReportError) {
  ...
  DEF_GRAPH(test_graph) {
    const auto data0 =
        OP_CFG(DATA).Attr(ATTR_NAME_INDEX, 0).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {10}).Build("data0");
    auto where = OP_CFG(WHERE).InCnt(1).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {-1, -1, -1, -1}).Build("where");
  ...
  setenv("RESOURCE_CONFIG_PATH", "fake_numa_config.json", 1);
  // new session & add graph
  map<AscendString, AscendString> options = {{"ge.inputShape", "data0:-1"}, {"ge.dynamicDims", "1;10;20"}};
  EXPECT_NE(OnlineInferDynCompile(graph, 1, options), SUCCESS);
...
TEST_F(OnlineInferTest, MinDimsSizeCheck) {
  ...
  // new session & add graph
  map<AscendString, AscendString> options = {
      {"ge.inputShape", "get_next:-1,3,16,16"}, {"ge.dynamicDims", "2"}, {"ge.dynamicNodeType", "0"}};
  EXPECT_NE(OnlineInferDynCompile(graph, 1, options), SUCCESS);
...
TEST_F(OnlineInferTest, MaxDimsSizeCheck) {
  ...
  DUMP_GRAPH_WHEN("PreRunAfterOptimize1");
  // new session & add graph
  map<AscendString, AscendString> options = {{"ge.inputShape", "get_next:-1,3,16,16"},
                                             {"ge.dynamicDims",
