...
TEST_F(SliceSchedulerTest, TestSliceScheduler_ForDynamicGraph) {
  ...
  runtime_stub_.GetSlogStub().Clear();
  {
    ...
    std::vector<std::string> expect_log_list = {"GetRemainingNodes completed. infer size:5, uninfer size:2",
    ...
    EXPECT_EQ(RunGraphAsync(session, graph_id, inputs, outputs), SUCCESS);
    std::vector<std::string> expect_log_list1 = {"ExecuteGraphWithStreamAsync GEP[ins_id:0] of EP[0] USER_GRAPH[4]",
                                                 "ExecuteGraphWithStreamAsync GEP[ins_id:1] of EP[1] USER_GRAPH[4]"};
    std::vector<std::string> unexpect_log_list1 = {"Start to compile GEP", "GetRemainingNodes completed"};
...
TEST_F(SliceSchedulerTest, TestSliceScheduler_ForDynamicGraph_MultiInstance) {
  ...
  runtime_stub_.GetSlogStub().Clear();
  {
    ...
    std::vector<std::string> expect_log_list = {"ExecuteGraphWithStreamAsync GEP[ins_id:0] of EP[0] USER_GRAPH[9]",
