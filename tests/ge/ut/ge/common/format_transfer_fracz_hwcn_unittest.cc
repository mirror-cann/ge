...
TEST_F(UtestFormatTransferFracZHwcn, fracz_to_hwcn_invalid_data_type_invalid_data_type) {
  ...
  TransArgs args{data, src_format, dst_format,      FORMAT_FRACTAL_Z, FORMAT_HWCN, FORMAT_RESERVED, FORMAT_RESERVED,
                 16,   16,         {16, 1, 32, 16}, {4, 4, 1, 1},     DT_UNDEFINED};
  TransResult result;
...
TEST_F(UtestFormatTransferFracZHwcn, uint8_1c_2n_padcn) {
  ...
  TransArgs args{reinterpret_cast<uint8_t *>(data),
                 ...
                 32,
                 32,
                 std::vector<int64_t>({9, 2, 16, 32}),
                 std::vector<int64_t>({3, 3, 30, 20}),
                 DT_UINT8};
  FormatTransferFracZHwcn transfer;
  TransResult result;
  EXPECT_EQ(transfer.TransFormat(args, result), SUCCESS);
