...
class UtestConstNodeMatcher : public testing::Test {
 public:
  static void SetUpTestSuite() {
    ...
    case_2_tensor_["int32_tensor_3_2"] =
        EsCreateConstInt32(graph_, int32_tensor_const_data.data(), int32_tensor_const_shape_3_2.data(),
                           int32_tensor_const_shape_3_2.size());
...
TEST_F(UtestConstNodeMatcher, EnableValueMatch_TargetConst_NoWeight_Invalid_Miss) {
  auto pattern_graph_ = std::unique_ptr<EsCGraphBuilder, void (*)(EsCGraphBuilder *)>(EsCreateGraphBuilder("pattern"),
                                                                                      EsDestroyGraphBuilder);
  ...
  AttrUtils::MutableTensor(NodeAdapter::GNode2Node(int32_scaler_const_target->GetProducer())->GetOpDesc(),
                           ge::ATTR_NAME_WEIGHTS, weight_backup);

  AttrUtils::ClearAllAttrs(NodeAdapter::GNode2Node(int32_scaler_const_target->GetProducer())->GetOpDesc());
  ConstantMatcher matcher(true, false);
  EXPECT_FALSE(matcher.IsMatch(NodeAdapter::GNode2Node(int32_scaler_const->GetProducer()),
                               NodeAdapter::GNode2Node(int32_scaler_const_target->GetProducer())));
}
...
TEST_F(UtestConstNodeMatcher, EnableValueMatch_PatternConst_NoWeight_Match) {
  ...
  EXPECT_TRUE(matcher.IsMatch(NodeAdapter::GNode2Node(int32_scaler_const->GetProducer()),
                              NodeAdapter::GNode2Node(int32_scaler_const_target->GetProducer())));
}
