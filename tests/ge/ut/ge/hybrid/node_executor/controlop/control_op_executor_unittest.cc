...
TEST_F(UtestControlOpExecutor, test_WhileOpNodeTask) {
  ...
  // this attr is required, otherwise task_context on node state cannot init successfully
  ...
  subgraph_context.all_outputs_ =
      std::vector<TensorValue>({TensorValue(std::shared_ptr<TensorBuffer>(tensor_buffer1))});
  ...
  ASSERT_NE(dynamic_cast<WhileOpNodeTask *>(task.get())->ExecuteOneLoop(*node_state->GetTaskContext(), is_continue),
            SUCCESS);

  ASSERT_EQ(dynamic_cast<WhileOpNodeTask *>(task.get())->MoveOutputs2Inputs(*node_state->GetTaskContext()),
            SUCCESS);  // ??
