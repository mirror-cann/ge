...
class UtestUnchangedTransposeRemovePass : public Test {
 protected:
  ...
  void EXPECT_TestUnchangedTransposeRemove(vector<int64_t> &data_vec, vector<inner_dim_type> &dim_value_vec,
  ...
  template <typename inner_data_type, DataType data_type, typename inner_dim_type, DataType dim_type>
  void EXPECT_TestChangedTransposeRemove(vector<int64_t> &data_vec, vector<inner_dim_type> &dim_value_vec,
                                         vector<int64_t> &result_vec) {
    ComputeGraphPtr graph = std::make_shared<ComputeGraph>("default");
