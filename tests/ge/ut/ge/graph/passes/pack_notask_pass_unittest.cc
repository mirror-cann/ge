...
static Graph BuildPackGraph(Format storage_format, Format origin_format, const std::vector<int64_t> &origin_shape,
                            const std::vector<int64_t> &shape) {
  DEF_GRAPH(g1) {
    auto data1 = OP_DATA(DATA).TensorDesc(FORMAT_ND, DT_FLOAT16, {256, 12288}).Build("data_1");
    auto data2 = OP_DATA(DATA).TensorDesc(FORMAT_ND, DT_FLOAT16, {256, 12288}).Build("data_2");
...
static Graph BuildPackGraphWithDataInput(Format storage_format, Format origin_format,
                                         const std::vector<int64_t> &origin_shape, const std::vector<int64_t> &shape) {
  DEF_GRAPH(g1) {
    auto data1 = OP_DATA(DATA).TensorDesc(storage_format, DT_FLOAT16, shape).Build("data_1");
    data1->MutableOutputDesc(0)->SetOriginFormat(origin_format);
    data1->MutableOutputDesc(0)->SetOriginShape(GeShape(origin_shape));
    auto data2 = OP_DATA(DATA).TensorDesc(storage_format, DT_FLOAT16, shape).Build("data_2");
...
TEST_F(UtestPackNotaskPass, pack_axis0_nd_success) {
  auto train_graph = BuildPackGraph(FORMAT_ND, FORMAT_ND, {2048, 12288}, {2048, 12288});
...
TEST_F(UtestPackNotaskPass, pack_axis1_dim0_is1_success) {
  auto train_graph = BuildPackGraph(FORMAT_ND, FORMAT_ND, {1, 12288}, {1, 12288});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
...
TEST_F(UtestPackNotaskPass, pack_axis1_dim0_not1_fail) {
  auto train_graph = BuildPackGraph(FORMAT_ND, FORMAT_ND, {2, 12288}, {2, 12288});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 1);
...
TEST_F(UtestPackNotaskPass, pack_pre_node_is_data_fail) {
  auto train_graph = BuildPackGraphWithDataInput(FORMAT_ND, FORMAT_ND, {2048, 12288}, {2048, 12288});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_tensor_not_aligned_fail) {
  ...
  auto train_graph = BuildPackGraph(FORMAT_ND, FORMAT_ND, {3, 5}, {3, 5});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
static Graph BuildSingleInputPackGraph(Format storage_format, Format origin_format,
  ...
  DEF_GRAPH(g1) {
    ...
    relu1->MutableOutputDesc(0)->SetOriginShape(GeShape(origin_shape));
    relu1->MutableOutputDesc(0)->SetShape(GeShape(shape));
    auto pack = OP_CFG("Pack").InCnt(1).OutCnt(1).Build("pack");
    pack->MutableInputDesc(0)->SetOriginFormat(origin_format);
    pack->MutableInputDesc(0)->SetFormat(storage_format);
    pack->MutableInputDesc(0)->SetOriginShape(GeShape(origin_shape));
    pack->MutableInputDesc(0)->SetShape(GeShape(shape));
...
TEST_F(UtestPackNotaskPass, pack_axis_equal_rank_all_ones_success) {
  auto train_graph = BuildSingleInputPackGraph(FORMAT_ND, FORMAT_ND, {1, 1, 1}, {1, 1, 1});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
...
TEST_F(UtestPackNotaskPass, pack_format_fractal_nz_success) {
  auto train_graph = BuildPackGraph(FORMAT_FRACTAL_NZ, FORMAT_ND, {1, 2048}, {1, 128, 16});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_format_fractal_nz_with_c0_success) {
  const auto composite_format = static_cast<Format>(ge::GetFormatFromC0(static_cast<int32_t>(FORMAT_FRACTAL_NZ), 5));
  auto train_graph = BuildPackGraph(composite_format, FORMAT_ND, {1, 2048}, {1, 128, 16});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_format_nc1hwc0_reject) {
  auto train_graph = BuildPackGraph(FORMAT_NC1HWC0, FORMAT_NCHW, {1, 32, 16, 16}, {1, 2, 16, 16, 16});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_format_fractal_z_reject) {
  auto train_graph = BuildPackGraph(FORMAT_FRACTAL_Z, FORMAT_NCHW, {1, 32, 16, 16}, {1, 2, 16, 16, 16});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_next_node_has_notask_fail) {
  ...
  DEF_GRAPH(g1) {
    ...
    auto relu_next = OP_CFG(RELU).InCnt(1).OutCnt(1).Build("relu_next");
    CHAIN(NODE(data1)->NODE(relu1)->EDGE(0, 0)->NODE(pack));
    CHAIN(NODE(data2)->NODE(relu2)->EDGE(0, 1)->NODE(pack));
...
TEST_F(UtestPackNotaskPass, pack_lxfusion_op_skip) {
  DEF_GRAPH(g1) {
    ...
    auto pack = OP_CFG("Pack").InCnt(2).OutCnt(1).Build("pack_lxslice_0");
    pack->MutableInputDesc(0)->SetOriginFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetOriginShape(GeShape({2048, 12288}));
    pack->MutableInputDesc(0)->SetShape(GeShape({2048, 12288}));
...
TEST_F(UtestPackNotaskPass, pack_scalar_input_fail) {
  DEF_GRAPH(g1) {
    auto data1 = OP_DATA(DATA).TensorDesc(FORMAT_ND, DT_FLOAT16, {1}).Build("data_1");
    auto data2 = OP_DATA(DATA).TensorDesc(FORMAT_ND, DT_FLOAT16, {1}).Build("data_2");
    auto relu1 = OP_CFG(RELU).InCnt(1).OutCnt(1).Build("relu_1");
    relu1->MutableOutputDesc(0)->SetOriginFormat(FORMAT_ND);
    relu1->MutableOutputDesc(0)->SetFormat(FORMAT_ND);
...
TEST_F(UtestPackNotaskPass, pack_format_nhwc_public_success) {
  auto train_graph = BuildPackGraph(FORMAT_NHWC, FORMAT_NHWC, {2048, 12288}, {2048, 12288});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 0);
...
TEST_F(UtestPackNotaskPass, pack_pre_node_is_variable_fail) {
  DEF_GRAPH(g1) {
    auto var1 = OP_CFG(VARIABLE).TensorDesc(FORMAT_ND, DT_FLOAT16, {2048, 12288}).InCnt(0).OutCnt(1).Build("var_1");
    auto var2 = OP_CFG(VARIABLE).TensorDesc(FORMAT_ND, DT_FLOAT16, {2048, 12288}).InCnt(0).OutCnt(1).Build("var_2");
    auto pack = OP_CFG("Pack").InCnt(2).OutCnt(1).Build("pack");
    pack->MutableInputDesc(0)->SetOriginFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetOriginShape(GeShape({2048, 12288}));
...
TEST_F(UtestPackNotaskPass, pack_pre_node_is_const_fail) {
  DEF_GRAPH(g1) {
    auto const1 = OP_CFG(CONSTANT).TensorDesc(FORMAT_ND, DT_FLOAT16, {2048, 12288}).InCnt(0).OutCnt(1).Build("const_1");
    auto const2 = OP_CFG(CONSTANT).TensorDesc(FORMAT_ND, DT_FLOAT16, {2048, 12288}).InCnt(0).OutCnt(1).Build("const_2");
    auto pack = OP_CFG("Pack").InCnt(2).OutCnt(1).Build("pack");
    pack->MutableInputDesc(0)->SetOriginFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetFormat(FORMAT_ND);
    pack->MutableInputDesc(0)->SetOriginShape(GeShape({2048, 12288}));
...
TEST_F(UtestPackNotaskPass, pack_fractal_nz_padding_mismatch_fail) {
  auto train_graph = BuildPackGraph(FORMAT_FRACTAL_NZ, FORMAT_ND, {1, 30}, {1, 2, 16, 16});
  auto compute_graph = GraphUtilsEx::GetComputeGraph(train_graph);
  auto pack_node = compute_graph->FindNode("pack");
  auto op_desc = pack_node->GetOpDesc();
  (void)ge::AttrUtils::SetInt(op_desc, "axis", 1);
