...
class TestTensor:
    ...
    def test_create_tensor_double_data_invalid():
        ...
        with pytest.raises(
            RuntimeError,
            match="DT_DOUBLE is not supported in python Tensor constructor",
        ):
    ...
    def test_transfer_ownership(tensor):
        ...
        with pytest.raises(
            RuntimeError,
            match="Tensor already has an new owner builder :graph, cannot transfer ownership again",
        ):
            tensor._transfer_ownership_when_pass_as_attr(builder)
        # 还原为了避免内存泄漏
