...
TEST_F(OpModelParserTest, TestDeserializeModel) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Init(_, _)).WillOnce(Return(1)).WillRepeatedly(Return(0));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetModelPartition(_, _))
      .WillOnce(Return(1))
      .WillRepeatedly(Return(0));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Load(_, _, _)).WillOnce(Return(1)).WillRepeatedly(Return(0));
...
TEST_F(OpModelParserTest, TestParseOpAttrsWithSingleOpScene) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetAllAttrs()).WillOnce(Invoke(GetAllAttrsWithSingleOp));
  ge::Model model;
  aclopAttr attr;
  OpModelParser::ParseOpAttrs(model, attr);
}
...
bool GetListNamedAttrs_invoke(ge::AttrUtils::ConstAttrHolderAdapter obj, const string &name,
                              vector<ge::GeAttrValue::NAMED_ATTRS> &value) {
  (void)obj;
  (void)name;
  value = g_value;
  return true;
...
TEST_F(OpModelParserTest, TestParseOpModel) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetBool(_, _, _)).WillRepeatedly(Invoke(GetBool_invoke1));
