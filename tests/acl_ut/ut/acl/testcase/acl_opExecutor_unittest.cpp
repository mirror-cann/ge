...
ge::Status LoadDynamicSingleOpV2_Invok(const std::string &model_name, const ge::ModelData &model_data, void *stream,
                                       ge::DynamicSingleOp **single_op, uint64_t model_id) {
  (void)model_name;
  (void)model_data;
  (void)stream;
  (void)model_id;
...
Status LoadSingleOpMock(const std::string &modelName, const ge::ModelData &modelData, void *stream,
...
Status LoadDynamicSingleOpMock(const std::string &modelName, const ge::ModelData &modelData, void *stream,
...
std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelDataReturnSuccess(
...
std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelDataCheckOption(
    const ge::ModelData &model_data, const gert::LoweringOption &optimize_option, ge::graphStatus &error_code) {
  (void)model_data;
...
std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelDataReturnFailed(
    const ge::ModelData &model_data, const gert::LoweringOption &optimize_option, ge::graphStatus &error_code) {
  (void)model_data;
  (void)optimize_option;
  error_code = ge::GRAPH_FAILED;
...
ge::graphStatus ExecuteFailed(const gert::ModelExecuteArg &arg, gert::Tensor **inputs, size_t input_num,
...
TEST_F(OpExecutorTest, TestCaseExecuteAsync) {
  ...
  ASSERT_EQ(aclopCreateKernel("BatchNorm", "kernel1", "kernel1", (void *)0x1000, 1024, ACL_ENGINE_AICORE, nullptr),
            ACL_SUCCESS);
  ASSERT_EQ(aclopUpdateParams("BatchNorm", 1, input_desc_, 1, output_desc_, nullptr), ACL_SUCCESS);
  int res1 = 0;
  size_t res2 = 0;
...
TEST_F(OpExecutorTest, TestCaseaclopExecWithHandle) {
  ...
  ASSERT_EQ(
      aclopCreateKernel("BatchNorm_Test1", "kernel1", "kernel1", (void *)0x1000, 1024, ACL_ENGINE_AICORE, nullptr),
      ACL_SUCCESS);
  ASSERT_EQ(aclopUpdateParams("BatchNorm_Test1", 1, input_desc_, 1, output_desc_, nullptr), ACL_SUCCESS);
  ...
  aclTensorDesc **const output_desc_tmp =
      const_cast<aclTensorDesc **>(const_cast<aclTensorDesc *const *>(output_desc_));
  aclDataBuffer **const outputs_tmp = const_cast<aclDataBuffer **>(const_cast<aclDataBuffer *const *>(outputs_));

  ASSERT_EQ(aclopExecuteV2("BatchNorm_Test1", 1, input_desc_tmp, inputs_tmp, 1, output_desc_tmp, outputs_tmp, nullptr,
                           nullptr),
            ACL_SUCCESS);
...
TEST_F(OpExecutorTest, TestCaseaclopUpdateParamsAging) {
  ...
  ASSERT_EQ(
      aclopCreateKernel("BatchNorm_Test3", "kernel1", "kernel1", (void *)0x1000, 1024, ACL_ENGINE_AICORE, nullptr),
      ACL_SUCCESS);
  ...
  ASSERT_EQ(
      aclopCreateKernel("BatchNorm_Test2", "kernel1", "kernel1", (void *)0x1000, 1024, ACL_ENGINE_AICORE, nullptr),
      ACL_SUCCESS);
  ASSERT_EQ(aclopUpdateParams("BatchNorm_Test2", 1, input_desc_, 1, output_desc_, nullptr), ACL_SUCCESS);
  ASSERT_EQ(acl::OpKernelSelector::GetInstance().kernelDescMap_.hashMap_.size(), 1);
...
TEST_F(OpExecutorTest, TestResourceManager) {
  ...
  const char *str = "str";
  ASSERT_EQ(resource_manager.GetMemory((void **)&str, 10), ACL_SUCCESS);
}
...
TEST_F(OpExecutorTest, TestExecutors) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclrtCtxGetCurrentDefaultStream(_)).WillOnce(Return(ACL_RT_SUCCESS));
  Executors::RemoveExecutor(nullptr);
}
...
TEST_F(OpExecutorTest, TestStaticExecuteAsyncWithOpHandle) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), ExecuteAsync(_, _, _, _, _)).WillRepeatedly(Return(SUCCESS));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Create(_)).WillRepeatedly(Invoke(CreateAllocators_succ));
...
TEST_F(OpExecutorTest, TestDynamicExecuteAsyncWithOpHandle1) {
  ...
  bool isDeduplicate = false;
  EXPECT_EQ(
      instance.RegisterModel(std::move(modelDef), AclOpResourceManager::GetInstance().opModels_, true, isDeduplicate),
      ACL_SUCCESS);
...
TEST_F(OpExecutorTest, DoRTV2ExecuteAsyncTest) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), CreateAndLoad(_, _)).WillRepeatedly(Invoke(CreateAndLoadNotNullptr));
  ret = OpExecutor::ExecuteAsync(aclOp, inputs, outputs, stream);
...
TEST_F(OpExecutorTest, AclInitCallbackFunc) {
  ...
  EXPECT_EQ(AclOpExecutorInitCallbackFunc(configBufferInvalid, strlen(configBufferInvalid), nullptr),
  ...
  EXPECT_EQ(AclOpResourceInitCallbackFunc(configBufferResInvalid, strlen(configBufferResInvalid), nullptr),
            ACL_ERROR_INVALID_PARAM);
...
TEST_F(OpExecutorTest, TestAclopExecute) {
  ...
  EXPECT_EQ(aclopExecute("BatchNorm_Test1", 1, inputDesc, inputs, 1, outputDesc, outputs, nullptr, nullptr),
