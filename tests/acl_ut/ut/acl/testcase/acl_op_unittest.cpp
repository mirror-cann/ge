...
TEST_F(OpApiTest, OpCompileTest) {
  ...
  aclSetTensorPlaceMent(const_cast<aclTensorDesc *>(input_desc_[0]), ACL_MEMTYPE_HOST);
  EXPECT_NE(aclopCompile("opType", 2, input_desc_, 1, output_desc_, opAttr, ACL_ENGINE_SYS, ACL_COMPILE_SYS, nullptr),
            ACL_SUCCESS);
  aclSetTensorPlaceMent(const_cast<aclTensorDesc *>(input_desc_[0]), ACL_MEMTYPE_HOST_COMPILE_INDEPENDENT);
  EXPECT_NE(aclopCompile("opType", 2, input_desc_, 1, output_desc_, opAttr, ACL_ENGINE_SYS, ACL_COMPILE_SYS, nullptr),
            ACL_SUCCESS);
  ...
  EXPECT_NE(aclopCompile("opType", 2, input_desc_, 1, output_desc_, opAttr, ACL_ENGINE_SYS, ACL_COMPILE_SYS, nullptr),
            ACL_SUCCESS);
  EXPECT_EQ(aclopCompile("opType", 2, input_desc_, 1, output_desc_, opAttr, ACL_ENGINE_SYS, compileFlag, nullptr),
...
TEST_F(OpApiTest, TestGetTensorSize) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclDataTypeSize(_)).WillOnce(Return((sizeof(int8_t))));
  int64_t shape[] = {16, 16};
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclDataTypeSize(_)).WillOnce(Return((sizeof(int32_t))));
  desc = aclCreateTensorDesc(ACL_FLOAT, 2, shape, ACL_FORMAT_ND);
  EXPECT_EQ(aclGetTensorDescSize(desc), 1024);
  aclDestroyTensorDesc(desc);

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclDataTypeSize(_)).WillOnce(Return((sizeof(int64_t))));
...
TEST_F(OpApiTest, TestTransTensorDescFormat) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), TransShape(_, _, _)).WillRepeatedly(Return(PARAM_INVALID));
  ASSERT_NE(aclTransTensorDescFormat(srcDesc, ACL_FORMAT_NC1HWC0, &dstDesc), ACL_SUCCESS);
  aclDestroyTensorDesc(srcDesc);
  Mock::VerifyAndClear((void *)(&MockFunctionTest::aclStubInstance()));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), TransShape(_, _, _)).WillRepeatedly(Return(SUCCESS));
...
TEST_F(OpApiTest, AclOpCompileTest) {
  ...
  ASSERT_NE(aclopUpdateParams(nullptr, 1, input_desc_, 1, output_desc_, opAttr), ACL_SUCCESS);
  ASSERT_NE(aclopUpdateParams("BatchNorm", 1, nullptr, 1, output_desc_, opAttr), ACL_SUCCESS);
  ASSERT_NE(aclopUpdateParams("BatchNorm", 1, input_desc_, 1, nullptr, opAttr), ACL_SUCCESS);
  aclopUnregisterCompileFunc("BatchNorm");
...
TEST_F(OpApiTest, TestAclOpSetKernelArgs) {
  ...
  string arg = "arg";
  EXPECT_EQ(aclopSetKernelArgs(&kernel_desc, kernel_id, 3, arg.c_str(), arg.size()), ACL_SUCCESS);
}
...
TEST_F(OpApiTest, aclopCreateKernelTest) {
  ...
  EXPECT_NE(
      aclopCreateKernel("opType2", "kernelId", "kernelName", (void *)0x1000, 1024, ACL_ENGINE_AICORE, deallocator),
      ACL_SUCCESS);
  EXPECT_NE(
      aclopCreateKernel("opType2", "kernelId", "kernelName", (void *)0x1000, 1024, ACL_ENGINE_AICORE, deallocator),
      ACL_SUCCESS);
  ...
  EXPECT_EQ(
      aclopCreateKernel("opType3", "kernelId", "kernelName", (void *)0x1000, 1024, ACL_ENGINE_VECTOR, deallocator),
      ACL_SUCCESS);
}
...
TEST_F(OpApiTest, TestAclOpInferShapeFail) {
  ...
  inputs[1] = aclCreateDataBuffer((void *)buf2, 16);
  std::string filePath = "";
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), RealPath(_)).WillRepeatedly(Return((filePath)));
...
TEST_F(OpApiTest, TestLoadOpsProtoFail) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), OpsProtoManager_Initialize(_)).WillRepeatedly(Return(false));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetOpsTypeList(_)).WillRepeatedly(Return((GRAPH_SUCCESS)));
...
TEST_F(OpApiTest, AclOpInferShapeTest) {
  ...
  EXPECT_NE(aclopInferShape("opType", 2, (aclTensorDesc **)input_desc_, inputsV2, 2, v2_empty_output_desc_, opAttr),
            ACL_SUCCESS);

  string filePath = "/path";
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), RealPath(_)).WillRepeatedly(Return((filePath)));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), OpsProtoManager_Initialize(_)).WillRepeatedly(Return(true));
  EXPECT_EQ(aclopInferShape("opType", 2, (aclTensorDesc **)input_desc_, inputsV2, 2, v2_empty_output_desc_, opAttr),
            ACL_SUCCESS);
...
TEST_F(OpApiTest, AclOpInferShapeTest02) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), RealPath(_)).WillRepeatedly(Return(opp_path));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), OpsProtoManager_Initialize(_)).WillRepeatedly(Return(true));
  acl::SetLoadOpsProtoFlag(false);
  EXPECT_EQ(aclopInferShape("opType", 2, (aclTensorDesc **)input_desc_, inputs, 2, v2_empty_output_desc_, opAttr),
            ACL_SUCCESS);
...
TEST_F(OpApiTest, AclOpInferShapeTest09) {
  ...
  MM_SYS_SET_ENV(MM_ENV_ASCEND_OPP_PATH, opp_path.c_str(), 1, mmRet);
  (void)mmRet;
  MM_SYS_SET_ENV(MM_ENV_ASCEND_CUSTOM_OPP_PATH,
                 (custom_opp_path_01 + ":" + custom_opp_path_invalid_01 + ":" + custom_opp_path_empty + ":" +
...
TEST_F(OpApiTest, aclopExecuteV2DataIsNull) {
  ...
  auto ret = aclopExecuteV2(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr, nullptr);
  ...
  inputs[0] = nullptr;
  inputs[1] = nullptr;
  ret = aclopExecuteV2(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr, nullptr);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
