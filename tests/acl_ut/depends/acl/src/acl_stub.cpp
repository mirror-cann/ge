...
aclError aclStub::aclrtMemcpy(void *dst, size_t destMax, const void *src, size_t count, aclrtMemcpyKind kind) {
  return ACL_SUCCESS;
}
...
aclError aclStub::aclrtMemcpyAsync(void *dst, size_t destMax, const void *src, size_t count, aclrtMemcpyKind kind,
                                   aclrtStream stream) {
  return static_cast<aclError>(ACL_ERROR_COMPILING_STUB_MODE);
}
...
aclError aclStub::aclrtLaunchCallback(aclrtCallback fn, void *userData, aclrtCallbackBlockType blockType,
                                      aclrtStream stream) {
  return static_cast<aclError>(ACL_ERROR_COMPILING_STUB_MODE);
}
...
aclError aclStub::aclrtAllocatorGetByStream(aclrtStream stream, aclrtAllocatorDesc *allocatorDesc,
...
aclError aclStub::aclInitCallbackRegister(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc, void *userData) {
  return ACL_SUCCESS;
}
aclError aclStub::aclInitCallbackUnRegister(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc) {
  return ACL_SUCCESS;
}
aclError aclStub::aclFinalizeCallbackRegister(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc,
                                              void *userData) {
  return ACL_SUCCESS;
}
aclError aclStub::aclFinalizeCallbackUnRegister(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc) {
  return ACL_SUCCESS;
}
...
void MockFunctionTest::ResetRtMocks() {
  ON_CALL(*this, aclrtMalloc).WillByDefault([this](void **devPtr, size_t size, aclrtMemMallocPolicy policy) {
    return aclStub::aclrtMalloc(devPtr, size, policy);
  });
  ON_CALL(*this, aclrtGetSocName).WillByDefault([this]() { return aclStub::aclrtGetSocName(); });
  ON_CALL(*this, aclrtFree).WillByDefault([this](void *devPtr) { return aclStub::aclrtFree(devPtr); });
  ON_CALL(*this, aclrtFree).WillByDefault([this](void *devPtr) { return aclStub::aclrtFree(devPtr); });
  ...
  ON_CALL(*this, aclrtCtxGetCurrentDefaultStream).WillByDefault([this](aclrtStream *stream) {
    return aclStub::aclrtCtxGetCurrentDefaultStream(stream);
  });
  ON_CALL(*this, aclrtCreateContext).WillByDefault([this](aclrtContext *context, int32_t deviceId) {
    return aclStub::aclrtCreateContext(context, deviceId);
  });
  ON_CALL(*this, aclrtDestroyContext).WillByDefault([this](aclrtContext context) {
    return aclStub::aclrtDestroyContext(context);
  });
}

void MockFunctionTest::ResetDataBufferMocks() {
  ON_CALL(*this, aclCreateDataBuffer).WillByDefault([this](void *data, size_t size) {
    return aclStub::aclCreateDataBuffer(data, size);
  });
  ON_CALL(*this, aclGetDataBufferAddr).WillByDefault([this](const aclDataBuffer *dataBuffer) {
    return aclStub::aclGetDataBufferAddr(dataBuffer);
  });
  ON_CALL(*this, aclDestroyDataBuffer).WillByDefault([this](const aclDataBuffer *dataBuffer) {
    return aclStub::aclDestroyDataBuffer(dataBuffer);
  });
  ON_CALL(*this, aclGetDataBufferSizeV2).WillByDefault([this](const aclDataBuffer *dataBuffer) {
    return aclStub::aclGetDataBufferSizeV2(dataBuffer);
  });
  ON_CALL(*this, aclGetDataBufferSize).WillByDefault([this](const aclDataBuffer *dataBuffer) {
...
aclError aclrtAllocatorGetByStream(aclrtStream stream, aclrtAllocatorDesc *allocatorDesc, aclrtAllocator *allocator,
                                   aclrtAllocatorAllocFunc *allocFunc, aclrtAllocatorFreeFunc *freeFunc,
                                   aclrtAllocatorAllocAdviseFunc *allocAdviseFunc,
                                   aclrtAllocatorGetAddrFromBlockFunc *getAddrFromBlockFunc) {
  return MockFunctionTest::aclStubInstance().aclrtAllocatorGetByStream(stream, allocatorDesc, allocator, allocFunc,
                                                                       freeFunc, allocAdviseFunc, getAddrFromBlockFunc);
}
