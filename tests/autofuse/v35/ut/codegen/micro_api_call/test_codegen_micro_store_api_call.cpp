...
TEST(CodegenKernel, StoreMicroApiCall_MaskReg_Store_FromUint16) {
  ge::AscGraph graph("test_mask_reg_store_graph");
  ...
  *store_op.y.strides = {s1, One};
  ...
  t1_load->attr.api.compute_type = ge::ComputeType::kComputeLoad;
  t1_load->attr.api.type = ge::ApiType::kAPITypeCompute;
  t1_load->attr.api.unit = ge::ComputeUnit::kUnitMTE2;
  ...
  t1_load->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  t1_load->outputs[0].attr.mem.tensor_id = 1;
  t1_load->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
  t1_load->outputs[0].attr.que.id = 2;
  t1_load->outputs[0].attr.opt.merge_scope = ge::kIdNone;
  ...
  ge->outputs[0].attr.mem.position = ge::Position::kPositionVecCalc;
...
TEST(CodegenKernel, StoreMicroApiCall_MaskReg_Store_FromUint64) {
  ...
  *store_op.y.strides = {s1, One};
  ...
  std::string store_result;
  store_call.Generate(tensor_mng, tpipe, cp, store_result);
  // Pack uint64 → uint32（1次），StoreAlign 使用 DIST_PACK4_B32 完成 uint32 → uint8
...
TEST(CodegenKernel, StoreMicroApiCall_VF_Store_Int64ToInt32) {
  ge::AscGraph graph("test_vf_store_dtype_graph");
