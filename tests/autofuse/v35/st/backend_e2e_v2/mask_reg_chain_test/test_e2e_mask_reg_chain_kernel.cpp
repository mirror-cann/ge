...
TEST_P(E2E_BackendMaskRegChain_Code, CalculateCorrect) {
  ...
  // Where0 输入 (INT32)
  int32_t *x2 = (int32_t *)AscendC::GmAlloc(test_size * sizeof(int32_t) + 32);
  int32_t *x3 = (int32_t *)AscendC::GmAlloc(test_size * sizeof(int32_t) + 32);

  // 阶段2: INT32 Compare -> Where (INT32→INT16)
  int32_t *x4 = (int32_t *)AscendC::GmAlloc(test_size * sizeof(int32_t) + 32);
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    bool mask0 = (x1[i] != 0);
    int32_t result0 = mask0 ? x2[i] : x3[i];

    // 阶段2: INT32 Ge0(result0>=x4) -> Where1(x5, x6)
