...
extern "C" __global__ __aicore__ void vf_scalar_fusion_comprehensive_test(GM_ADDR x0, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendVfScalarFusionComprehensive_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {
};
...
TEST_P(E2E_BackendVfScalarFusionComprehensive_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    // 计算期望输出
    // 图结构:
    // scalar(1.0) -> brc1 -> brc2 -> brc3
    //                                              \
    //                                               maximum0 = max(brc3, x0)
    //                                               minimum1 = min(brc3, x0)
    //                                               add2 = add(brc3, x0)
    //                                               add3 = add(maximum0, minimum1)
    //                                               minimum4 = min(add3, add2)
    //                                               maximum5 = max(minimum4, add2)
    // output = maximum5

    float f0 = rand_val;
    float s = scalar_val;

    // brc只是broadcast，值不变，所以 brc3_val = scalar_val = 1.0
    float brc3_val = s;

    float maximum0_val = MaxFloat(brc3_val, f0);
    float minimum1_val = MinFloat(brc3_val, f0);
    float add2_val = brc3_val + f0;
    float add3_val = maximum0_val + minimum1_val;
    float minimum4_val = MinFloat(add3_val, add2_val);
    float maximum5_val = MaxFloat(minimum4_val, add2_val);
  ...
  ICPU_RUN_KF(vf_scalar_fusion_comprehensive_test, tiling_data.block_dim, (uint8_t *)x0, (uint8_t *)y, nullptr, (uint8_t*)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendVfScalarFusionComprehensive_Code,
