...
void SetMemReuseIds(const std::vector<std::pair<std::string, int>> &id_map, ge::AscGraph &graph) {
  for (const auto &[name, id] : id_map) {
    graph.FindNode(name.c_str())->outputs[0].attr.mem.reuse_id = id;
  }
}

std::vector<ge::Expression> CalcTransposedStrides(const std::vector<ge::Expression> &dims,
                                                  const std::vector<int64_t> &perm) {
  std::vector<ge::Expression> strides(perm.size());
  ge::Expression stride = ge::Symbol(1);
...
Status Build2DTransposeAscendGraph(ge::AscGraph &graph, const std::vector<int64_t> &perm = {1, 0}) {
...
Status Build2DPadAscendGraph(ge::AscGraph &graph, const std::vector<int64_t> &perm = {1, 0}) {
  if (perm.size() != 2 || !IsValidPermutation(perm)) {
    return ge::FAILED;
  }
  auto s0 = graph.CreateSizeVar("s0");
...
Status BuildTransposeAscendGraph(ge::AscGraph &graph, const std::vector<int64_t> &perm = {0, 2, 1}) {
