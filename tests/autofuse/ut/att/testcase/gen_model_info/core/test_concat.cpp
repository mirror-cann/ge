...
namespace {
void InitDataNode(Data &node, int &exec_order, const std::vector<int64_t> &axis, ge::DataType dtype,
...
void InitLoadNode(Load &node, int &exec_order, const std::vector<int64_t> &axis, ge::DataType dtype,
                  const std::vector<ge::Expression> &repeats, const std::vector<ge::Expression> &strides) {
  node.attr.sched.exec_order = exec_order++;
  node.attr.sched.axis = axis;
  node.y.dtype = dtype;
...
void InitStoreNode(Store &node, int &exec_order, const std::vector<int64_t> &axis, ge::DataType dtype,
                   const std::vector<ge::Expression> &repeats, const std::vector<ge::Expression> &strides) {
  node.attr.sched.exec_order = exec_order++;
  node.attr.sched.axis = axis;
  node.y.dtype = dtype;
...
void InitOutputNode(Output &node, int &exec_order, const std::vector<int64_t> &axis, ge::DataType dtype,
                    const std::vector<ge::Expression> &repeats, const std::vector<ge::Expression> &strides) {
  node.attr.sched.exec_order = exec_order++;
  node.y.dtype = dtype;
  *node.y.axis = axis;
...
void InitConcatAxes(Concat &node, int &exec_order, const std::vector<int64_t> &axis, ge::DataType dtype,
                    const std::vector<ge::Expression> &repeats, const std::vector<ge::Expression> &strides) {
  node.attr.sched.exec_order = exec_order++;
  node.attr.sched.axis = axis;
  node.y.dtype = dtype;
...
void Concat_Normal_BeforeAutofuse(ascir::HintGraph &graph) {
  auto ONE = ge::sym::kSymbolOne, ZERO = ge::sym::kSymbolZero;
  auto A = ge::Symbol("A"), R = ge::Symbol("R"), BL = ge::Symbol(8, "BL");
  auto a = graph.CreateAxis("A", A), r = graph.CreateAxis("R", R), bl = graph.CreateAxis("BL", BL);
  auto axis = std::vector<int64_t>{a.id, r.id, bl.id};
  auto rep_ar = std::vector<ge::Expression>{A, R, ONE};
  auto str_ar = std::vector<ge::Expression>{R, ONE, ZERO};
  auto rep_a = std::vector<ge::Expression>{A, ONE, ONE};
  auto str_a = std::vector<ge::Expression>{ONE, ZERO, ZERO};
  int exec_order = 0;

  Data x1("x1", graph); InitDataNode(x1, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Load x1L("x1Local"); x1L.x = x1.y; InitLoadNode(x1L, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Data x2("x2", graph); InitDataNode(x2, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Load x2L("x2Local"); x2L.x = x2.y; InitLoadNode(x2L, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Data bias("bias", graph); InitDataNode(bias, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Load biasL("biasLocal"); biasL.x = bias.y; InitLoadNode(biasL, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);

  Concat mean("mean");
  mean.x = {x1L.y, x2L.y, biasL.y};
  InitConcatAxes(mean, exec_order, axis, ge::DT_FLOAT, rep_ar, str_ar);
  Store x_out("x_out"); x_out.x = mean.y; InitStoreNode(x_out, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Store mean_out("mean_out"); mean_out.x = mean.y; InitStoreNode(mean_out, exec_order, axis, ge::DT_FLOAT, rep_a, str_a);

  Data one("one", graph);
  InitDataNode(one, exec_order, axis, ge::DT_FLOAT, {ONE, ONE, BL}, {ZERO, ZERO, ONE});
  Concat rstd("rstd");
  rstd.x = {mean.y, mean.y, one.y};
  InitConcatAxes(rstd, exec_order, axis, ge::DT_FLOAT, rep_ar, str_ar);
  Store rstd_out("rstd_out"); rstd_out.x = rstd.y; InitStoreNode(rstd_out, exec_order, axis, ge::DT_FLOAT, rep_a, str_a);

  auto rep_r = std::vector<ge::Expression>{ONE, R, ONE};
  auto str_r = std::vector<ge::Expression>{ZERO, ONE, ZERO};
  Data beta("beta", graph); InitDataNode(beta, exec_order, axis, ge::DT_FLOAT16, rep_r, str_r);
  Load betaL("betaLocal"); betaL.x = beta.y; InitLoadNode(betaL, exec_order, axis, ge::DT_FLOAT16, rep_r, str_r);
  Data gamma("gamma", graph); InitDataNode(gamma, exec_order, axis, ge::DT_FLOAT16, rep_r, str_r);
  Load gammaL("gammaLocal"); gammaL.x = gamma.y; InitLoadNode(gammaL, exec_order, axis, ge::DT_FLOAT16, rep_r, str_r);
  ...
  *concat.y.axis = axis, *concat.y.repeats = rep_ar, *concat.y.strides = str_ar;
  Store cat_out("cat_out"); cat_out.x = y.y; InitStoreNode(cat_out, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Store y_out("y_out"); y_out.x = y.y; InitStoreNode(y_out, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);

  Output buf1("buf1"); buf1.x = x_out.y; InitOutputNode(buf1, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Output buf2("buf2"); buf2.x = mean_out.y; InitOutputNode(buf2, exec_order, axis, ge::DT_FLOAT, rep_a, str_a);
  Output buf3("buf3"); buf3.x = rstd_out.y; InitOutputNode(buf3, exec_order, axis, ge::DT_FLOAT, rep_a, str_a);
  Output buf("buf"); buf.x = y_out.y; InitOutputNode(buf, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
  Output buf4("buf4"); buf4.x = cat_out.y; InitOutputNode(buf4, exec_order, axis, ge::DT_FLOAT16, rep_ar, str_ar);
}
...
void Concat_Normal_AfterScheduler(ascir::HintGraph &graph) {
  ...
  graph.FindNode("mean")->attr.api.unit = ge::ComputeUnit::kUnitVector;
  graph.FindNode("rstd")->attr.api.unit = ge::ComputeUnit::kUnitVector;
  graph.FindNode("rstd_out")->attr.api.unit = ge::ComputeUnit::kUnitVector;
  ...
  graph.FindNode("concat")->attr.api.unit = ge::ComputeUnit::kUnitVector;
}
...
void Concat_Normal_AfterQueBufAlloc(ascir::HintGraph &graph) {
  ...
  SetQueueOutput(graph, "x1Local", tensor_id, x1Que, ge::Position::kPositionVecIn);
  SetQueueOutput(graph, "x2Local", tensor_id, x2Que, ge::Position::kPositionVecIn);
  SetQueueOutput(graph, "biasLocal", tensor_id, biasQue, ge::Position::kPositionVecIn);
  SetQueueOutput(graph, "mean", tensor_id, meanQue, ge::Position::kPositionVecOut);
  ...
  SetQueueOutput(graph, "rstd", tensor_id, yQue, ge::Position::kPositionVecOut);

  SetGmOutput(graph, "rstd_out");
  SetGmOutput(graph, "beta");

  SetQueueOutput(graph, "betaLocal", tensor_id, betaQue, ge::Position::kPositionVecIn);

  SetGmOutput(graph, "gamma");

  SetQueueOutput(graph, "gammaLocal", tensor_id, gammaQue, ge::Position::kPositionVecIn);
  SetQueueOutput(graph, "y", tensor_id, yQue, ge::Position::kPositionVecOut);

  SetGmOutput(graph, "y_out");
  SetQueueOutput(graph, "concat", tensor_id, yQue, ge::Position::kPositionVecOut);
...
void GenerateConcatTilingData(const ascir::FusedScheduledResult &fused_result,
  ...
  EXPECT_EQ(generator.GenTilingCode("Concat", all_model_infos, generator_config, tiling_res), ge::SUCCESS);
  std::ofstream oss;
...
void PrepareStubAndCompile() {
  auto ret = std::system(
      std::string("cp ").append(TOP_DIR).append("/tests/autofuse/st/att/testcase/stub/op_log.h ./ -f").c_str());
  EXPECT_EQ(ret, 0);
