...
TEST(CodegenKernel, TrueDivExtendWhenInputIsFLOAT) {
  ...
  Load load_op("load");
  Load load_op2("load2");
  ge::ascir_op::TrueDiv true_div_op("true_div");
  ...
  *load_op2.y.strides = {s1, One};
  ...
  true_div->attr.api.unit = ge::ComputeUnit::kUnitVector;
  true_div->attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr(), 0}};
  true_div->outputs[0].attr.dtype = ge::DT_FLOAT;
  true_div->outputs[0].attr.mem.position = ge::Position::kPositionVecOut;
  true_div->outputs[0].attr.mem.tensor_id = 2;
  true_div->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
  true_div->outputs[0].attr.que.id = 2;
  true_div->outputs[0].attr.opt.merge_scope = ge::kIdNone;
