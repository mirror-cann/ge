...
int32_t CopyStubsAndCompile(const std::string &main_cpp, const std::string &tiling_cpp,
  ...
  ret = std::system(
      std::string("cp -r ")
          .append(TOP_DIR)
          .append("/tests/autofuse/st/att/testcase/source_mirror/generator/generator_utils/kernel_tiling/ ./ -f")
          .c_str());
  EXPECT_EQ(ret, 0);
...
int32_t CopyAndRunTransposeTest() {
  ...
  ret = std::system(std::string("cp -r ")
                        .append(TOP_DIR)
                        .append("/tests/autofuse/st/att/testcase/stub/tensor.h ./graph/ -f")
                        .c_str());
  EXPECT_EQ(ret, 0);
  ret = std::system(std::string("cp -r ")
                        .append(TOP_DIR)
                        .append("/tests/autofuse/st/att/testcase/source_mirror/generator/generator_utils/lib/ ./ -f")
                        .c_str());
  EXPECT_EQ(ret, 0);
  ret = std::system(
      std::string("cp -r ")
