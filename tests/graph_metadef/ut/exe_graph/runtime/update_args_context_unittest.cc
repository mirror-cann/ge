...
class MockArgsHandler : public ArgsHandler {
 public:
  const KernelArgs* MallocReadOnlyDevArgs(void *host_args, size_t args_size) override { return nullptr; }
  ...
  void SetHostArgs(const std::deque<KernelArgs>& args) { host_args_ = args; }
  void SetDeviceArgs(const std::deque<KernelArgs>& args) { device_args_ = args; }
