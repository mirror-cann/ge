...
Status GetWorkspaceBaseDir(std::string &base_dir) {
  base_dir.clear();
  GE_ASSERT_SUCCESS(ge::om2::GetAscendWorkPath(base_dir));
  if (base_dir.empty()) {
    ...
    const std::string file_path = ge::om2::RealPath(path_env);
...
Status CreateOm2WorkspaceDir(std::string &ws_dir) {
  ...
  GE_ASSERT_TRUE(ge::om2::CreateDir(ws_dir) == 0, "Failed to create om2 work dir: [%s].", ws_dir.c_str());
  return ge::GRAPH_SUCCESS;
}
