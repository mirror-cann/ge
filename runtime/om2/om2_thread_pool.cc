  ...
  for (std::thread &thd : pool_) {
    if (thd.joinable()) {
      try {
        ...
        GELOGW("[OM2] system_error when joining thread pool worker.");
      } catch (...) {
        GELOGW("[OM2] exception when joining thread pool worker.");
      }
    }
  }
}
...
void ThreadPool::ThreadFunc(ThreadPool *const thread_pool, uint32_t thread_idx) {
  ...
  if (!thread_pool->thread_name_prefix_.empty()) {
    const auto thread_name = thread_pool->thread_name_prefix_ + std::to_string(thread_idx);
    const int32_t set_ret = pthread_setname_np(pthread_self(), thread_name.c_str());
    GELOGD("[OM2] set thread name to [%s], ret=%d", thread_name.c_str(), set_ret);
  }
