...
Status StreamSwitchNodeTask::Init(const HybridModel &model, const NodePtr &node) {
  ...
  if (it == kCompHandle.end()) {
    GELOGE(INTERNAL_ERROR, "[Check][Param] node:%s(%s) Get Condition: %u handle failed.", node->GetName().c_str(),
           node->GetType().c_str(), val);
    return INTERNAL_ERROR;
  }
...
Status StreamSwitchNodeTask::ExecuteAsync(TaskContext &context, const std::function<void()> &done_callback) {
  ...
  GELOGI("[%s] Done executing successfully, pred value: %ld, comp value: %ld, switch index: %d.", context.GetNodeName(),
         pred_value, comp_value, static_cast<int32_t>(switch_idx));
  return SUCCESS;
}
...
Status StreamMergeNodeTask::ExecuteAsync(TaskContext &context, const std::function<void()> &done_callback) {
  ...
  if (out_y->GetSize() > 0UL) {
    GE_CHK_ACL_RET(aclrtMemcpyAsync(out_y->MutableData(), out_y->GetSize(), &idx, sizeof(idx),
