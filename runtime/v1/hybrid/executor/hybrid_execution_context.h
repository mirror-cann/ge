...
#define IGNORE_NONNULL_WARNING _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wnonnull-compare\"")
    ...
    if (ge::hybrid::GraphExecutionContext::profiling_level > 0) {                                                  \
      if (((context) != nullptr) && ((context)->profiler != nullptr)) {                                            \
        if ((node_name) != nullptr) {                                                                              \
          (context)->profiler->RecordEvent((evt_type), "tid:%lu [%s@%ld] [%s] " fmt, GeLog::GetTid(), (node_name), \
                                           (context)->iteration, (category), ##__VA_ARGS__);                       \
        } else {                                                                                                   \
          (context)->profiler->RecordEvent((evt_type), "tid:%lu [%s] " fmt, GeLog::GetTid(), (category),           \
                                           ##__VA_ARGS__);                                                         \
        }                                                                                                          \
      }                                                                                                            \
    }                                                                                                              \
