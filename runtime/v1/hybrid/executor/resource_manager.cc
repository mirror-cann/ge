...
Status ResourceManager::InitDataFlowResource() {
  for (const auto node_item : graph_item_->GetAllNodes()) {
    ...
    if (!(AttrUtils::GetInt(node_item->GetOpDesc(), ATTR_NAME_DATA_FLOW_HANDLE, data_flow_handle))) {
      REPORT_INNER_ERR_MSG("E19999", "Failed to get handle for [%s(%s)].", node_item->NodeName().c_str(),
                           node_item->NodeType().c_str());
      GELOGE(INTERNAL_ERROR, "[Get][Attr] Failed for [%s(%s)].", node_item->NodeName().c_str(),
             node_item->NodeType().c_str());
    ...
    if (data_flow_kernels_.count(node_item->NodeType()) == 0UL) {
      ...
      if (kernel == nullptr) {
        ...
        GELOGE(INTERNAL_ERROR, "[Create][Kernel] failed for [%s(%s)], handle:%ld.", node_item->NodeName().c_str(),
               node_item->NodeType().c_str(), data_flow_handle);
        return INTERNAL_ERROR;
      }
    ...
    if (data_flow_resources_.count(data_flow_handle) == 0UL) {
      ...
      if (res == nullptr) {
        REPORT_INNER_ERR_MSG("E19999", "Create res failed for [%s(%s)], handle:%" PRId64 ".",
                             node_item->NodeName().c_str(), node_item->NodeType().c_str(), data_flow_handle);
        GELOGE(INTERNAL_ERROR, "[Create][Res] failed for [%s(%s)], handle:%ld.", node_item->NodeName().c_str(),
               node_item->NodeType().c_str(), data_flow_handle);
        return INTERNAL_ERROR;
      }
    ...
    GELOGD("Init data flow for [%s(%s)], handle:%ld.", node_item->NodeName().c_str(), node_item->NodeType().c_str(),
           data_flow_handle);
  }
  return SUCCESS;
}
