...
class ProxyEventManager {
 public:
  ...
  static Status AddGroup(int32_t device_id, const std::string &group_name, pid_t pid, bool is_admin, bool is_alloc);
  ...
  static Status CopyQMbuf(int32_t device_id, uint64_t dest_addr, uint32_t dest_len, uint32_t queue_id);

 private:
  static Status SubmitEventSync(int32_t device_id, uint32_t sub_event_id, char_t *msg, size_t msg_len,
