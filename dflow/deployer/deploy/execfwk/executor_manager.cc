...
std::string ExecutorManager::ExecutorKeyGetter::GetKey(const ExecutorManager::ExecutorKey &executor_key) {
  std::string key = std::to_string(executor_key.device_id) + "_" + std::to_string(executor_key.device_type) + "_" +
                    std::to_string(executor_key.context_id) + "_" + executor_key.engine_name + "_" +
                    std::to_string(executor_key.process_id) + "_" + executor_key.rank_id;
  return key;
}

std::string ExecutorManager::UdfExecutorKeyGetter::GetKey(const ExecutorManager::ExecutorKey &executor_key) {
  std::string key = std::to_string(executor_key.device_id) + "_" + std::to_string(executor_key.device_type) + "_" +
                    std::to_string(executor_key.context_id) + "_" + executor_key.engine_name;
  return key;
}
...
Status ExecutorManager::GetOrCreateExecutorClient(const ExecutorManager::ExecutorKey &executor_key,
  ...
  const auto &process_id = executor_key.process_id;
  {
    ...
    GELOGI(
        "Executor process does not exist, start to create context_id = %ld, "
    ...
    std::unique_ptr<PneExecutorClient> executor_client =
        PneExecutorClientFactory::GetInstance().CreateClient(engine_name, executor_key.is_proxy, device_id);
  ...
  GELOGI(
      "Executor process started successfully, context_id = %ld, "
      "device id = %d, device_type = %d, engine_name = %s, rank_id = %s, process_id = %d",
      context_id, device_id, executor_key.device_type, engine_name.c_str(), rank_id.c_str(), process_id);
  return SUCCESS;
}
...
Status ExecutorManager::GetExecutorClient(const ExecutorManager::ExecutorKey &executor_key,
  ...
  GELOGW(
      "Executor process does not exist, context_id = %ld, device id = %d, "
      "engine name = %s, rank id = %s, process_id = %d",
      executor_key.context_id, executor_key.device_id, executor_key.engine_name.c_str(), executor_key.rank_id.c_str(),
...
void ExecutorManager::GetExecutorStatus(deployer::DeployerResponse &response,
  ...
  for (auto &executor_client : executor_clients_) {
    ...
    if (status == ProcStatus::NORMAL) {
      ...
      GELOGW("Subprocess[%d] exited, device id[%d], name[%s].", key.process_id, key.device_id, key.engine_name.c_str());
...
void ExecutorManager::Finalize() {
  ...
  for (auto &executor_client : executor_clients_) {
    threads.emplace_back(
        [&executor_client]() { GE_CHK_STATUS(executor_client.second->Finalize(), "Finalize failed"); });
  }
