...
int32_t CreateDir(const std::string &directory_path, uint32_t mode) {
  LLM_CHK_BOOL_EXEC(!directory_path.empty(), REPORT_INNER_ERR_MSG("E18888", "directory path is empty, check invalid");
...
ge::Status LLMFileSaver::CheckPathValid(const std::string &file_path) {
  ...
  if (file_path.size() >= static_cast<size_t>(MMPA_MAX_PATH)) {
    LLMLOGE(ge::FAILED, "[Check][FilePath]Failed, file path's length:%zu >= mmpa_max_path:%d", file_path.size(),
            MMPA_MAX_PATH);
    REPORT_INNER_ERR_MSG("E19999",
                         "Check file path failed, file path's length:%zu >= "
                         "mmpa_max_path:%d",
...
ge::Status LLMFileSaver::OpenFile(int32_t &fd, const std::string &file_path, const bool append) {
  if (CheckPathValid(file_path) != ge::SUCCESS) {
    LLMLOGE(ge::FAILED, "[Check][FilePath]Check output file failed, file_path:%s.", file_path.c_str());
    REPORT_INNER_ERR_MSG("E19999", "Check output file failed, file_path:%s.", file_path.c_str());
    return ge::FAILED;
  }
...
ge::Status LLMFileSaver::WriteData(const void *const data, uint64_t size, const int32_t fd) {
  ...
  while (size > 0U) {
    ...
    LLM_ASSERT_TRUE(((write_count != EN_INVALID_PARAM) && (write_count != EN_ERROR)), "Write data failed, errno: %lld",
                    write_count);
    seek = PtrAdd<uint8_t>(seek, static_cast<size_t>(size), write_count);
    LLM_ASSERT_TRUE(size >= static_cast<uint64_t>(write_count), "Write data failed, errno: %lld, size: %u", write_count,
                    size);
    size -= write_count;
  }
