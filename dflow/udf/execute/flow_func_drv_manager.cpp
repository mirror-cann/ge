...
int32_t FlowFuncDrvManager::Init() const {
  ...
  if (drv_ret != DRV_ERROR_NONE) {
    UDF_LOG_ERROR("Failed to attach device[%u] for eSched, drv_ret=%d.", device_id, static_cast<int32_t>(drv_ret));
...
int32_t FlowFuncDrvManager::InitMemGroup() const {
  ...
  int32_t cpy_ret =
      memcpy_s(query_input.grpQueryGroupId.grpName, BUFF_GRP_NAME_LEN, mem_group_name.c_str(), mem_group_name.size());
  if (cpy_ret != EOK) {
    UDF_LOG_ERROR("copy group name failed, dst len=%d, src len=%zu, group name=%s", BUFF_GRP_NAME_LEN,
                  mem_group_name.size(), mem_group_name.c_str());
...
int32_t FlowFuncDrvManager::InitBuff() const {
  ...
  for (size_t i = 0UL; i < buf_cfg.size(); ++i) {
    ...
    default_cfg.cfg[i] = {.cfg_id = static_cast<uint32_t>(i),
                          .total_size = cfg.total_size,
                          .blk_size = cfg.blk_size,
                          .max_buf_size = cfg.max_buf_size,
                          .page_type = (page_type | BUFF_SP_DVPP),
                          1,
                          0,
                          0,
                          0,
                          ...
                          {}};
    UDF_LOG_INFO(
        "Prepare to set buf config{ id[%lu] total_size[%u] blk_size[%u] max_buf_size[%u] "
        "page_type[%s|dvpp] }.",
        i, cfg.total_size, cfg.blk_size, cfg.max_buf_size, cfg.page_type.c_str());
  }
...
int32_t FlowFuncDrvManager::CreateSchedGroup() const {
  ...
  UDF_LOG_INFO(
      "create sched group success, main_sched_group_id=%u, worker_sched_group_id=%u, invoke_model_sched_group_id=%u.",
...
int32_t FlowFuncDrvManager::WaitBindHostPid(uint32_t wait_timeout) const {
  ...
  do {
    ...
    if ((drv_query_ret == DRV_ERROR_NONE)) {
      UDF_LOG_INFO("drvQueryProcessHostPid success, udf_pid=%d, chip_id=%u, vf_id=%u, host_pid=%u, cp_type=%u", udf_pid,
                   chip_id, vf_id, host_pid, cp_type);
      return FLOW_FUNC_SUCCESS;
    }
    if (drv_query_ret != DRV_ERROR_NO_PROCESS) {
      UDF_LOG_WARN("drvQueryProcessHostPid failed, drv_query_ret=%d, udf_pid=%u", static_cast<int32_t>(drv_query_ret),
                   udf_pid);
      return FLOW_FUNC_ERR_DRV_ERROR;
    }
