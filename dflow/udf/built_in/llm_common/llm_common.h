...
struct TransferKvReqInfo {
  ...
  uint64_t block_len = 0UL;  // when src is not block, dst is block, host need fill, PA: blockSize * hiddenSize, Non-PA:
                             // kvCacheTensorSize_
...
enum class LinkReqType : uint32_t { kUnlink = 0U, kLink = 1U, kServerUnlink = 2U };
...
int32_t CheckInputTensor(const std::vector<std::shared_ptr<FlowMsg>> &input_msgs, uint64_t msgs_min_size,
                         size_t target_pos, uint64_t expect_size, bool is_expect_min = false);
