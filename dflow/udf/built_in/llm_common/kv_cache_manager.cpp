...
std::shared_ptr<FlowMsg> KvCacheManager::QueryDecoderKvCache(const ReqKey &req_key) {
  ...
  if (iter == decoder_req_to_kv_cache_.end()) {
    uint64_t cached_req_id =
        decoder_req_to_kv_cache_.empty() ? UINT64_MAX : decoder_req_to_kv_cache_.begin()->first.req_id;
...
void KvCacheManager::SaveKvCacheForPrompt(const ReqKey &req_key, uint64_t cache_id,
...
FsmStatus KvCacheManager::ReleaseKvCacheForPrefix(const PrefixReqKey &prefix_req_key) {
  std::unique_lock<std::mutex> lock(mutex_);
  UDF_LOG_INFO("Release KV Cache for prefix, prefix_id=%lu, model_id=%lu", prefix_req_key.prefix_id,
               prefix_req_key.model_id);
  const auto iter = prefix_id_to_cache_id_.find(prefix_req_key);
  if (iter == prefix_id_to_cache_id_.end()) {
  ...
  int64_t tensor_size = -1;
  {
    ...
    for (auto &kv_tensor : cache_entry.kv_tensors) {
      ...
      if (new_kv_tensor == nullptr) {
        UDF_LOG_ERROR("Shink kv cache failed. prefix_id = %lu, Currently cache size:%lu", prefix_req_key.prefix_id,
                      KvCacheManager::GetInstance().GetPromptKvCacheCount());
        return FsmStatus::kFsmOutOfMemory;
      }
      auto rt_ret = halSdmaCopy(reinterpret_cast<uintptr_t>(new_kv_tensor->GetTensor()->GetData()), tensor_size,
