...
class LlmCommEntity {
 public:
  ...
  FsmStatus PullOrTransferCache(const TransferKvReqInfo &req_info, const PullKvReqInfo &pull_req_info,
 ...
 private:
  ...
  FsmStatus PullKvFromPrompt(const PullKvReqInfo &req_info, bool enable_paged_attention, const CacheEntry *cache_entry,
                             uint64_t start_tick);

  FsmStatus TransferCache(const TransferKvReqInfo &req_info, bool enable_paged_attention, const CacheEntry *cache_entry,
                          uint64_t start_tick);
  ...
  FsmStatus TransferCacheData(const TransferKvReqInfo &req_info, const CacheEntry *cache_entry);

  FsmStatus ReceiveKvCache(const PullKvReqInfo &req_info, const SyncKvMetaInfo &resp_info,
                           const CacheEntry *cache_entry);

  FsmStatus ReceiveSyncKvMetaInfo(const PullKvReqInfo &req_info, SyncKvMetaInfo &resp_info,
                                  const CacheEntry *cache_entry);
  ...
  FsmStatus CheckSyncKvMetaInfo(const PullKvReqInfo &req_info, SyncKvMetaInfo &resp_info,
                                const CacheEntry *cache_entry) const;
  std::vector<uintptr_t> GetRecvAddrs(const PullKvReqInfo &req_info, const SyncKvMetaInfo &resp_info,
                                      const CacheEntry *cache_entry);
  FsmStatus ReceiveKvCacheAsync(const PullKvReqInfo &req_info, const SyncKvMetaInfo &resp_info,
