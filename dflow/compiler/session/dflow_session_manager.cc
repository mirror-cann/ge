...
SessionPtr DFlowSessionManager::CreateSession(const std::map<std::string, std::string> &options, uint64_t &session_id) {
  if (!init_flag_) {
...
SessionPtr DFlowSessionManager::GetSession(uint64_t session_id) {
  if (!init_flag_) {
    ...
    REPORT_INNER_ERR_MSG(
        "E19999", "GetSession fail for DFlowSession manager is not initialized, session_id:%" PRIu64 ".", session_id);
    return nullptr;
  }
  const std::lock_guard<std::mutex> lock(mutex_);
  const auto it = session_manager_map_.find(session_id);
