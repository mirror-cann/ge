...
class FMK_FUNC_HOST_VISIBILITY ParserOperator {
 public:
  ...
  const std::vector<ge::GeTensorDesc> &GetInputTensorDesc() const {
    return input_descs_;
  }

  const std::vector<ge::GeTensorDesc> &GetOutputTensorDesc() const {
  ...
  bool HasAttr(const std::string &name) const {
    return op_attrs_.find(name) != op_attrs_.end();
  }
