...
aclError AclOpResourceManager::HandleMaxOpQueueConfig(const char_t *const configBuffer) {
  ...
  bool found = false;
  const aclError ret =
      acl::JsonParser::GetJsonCtxByKeyFromBuffer(configBuffer, maxOpNumStr, ACL_MAX_OPQUEUE_NUM, found);
  if (ret != ACL_SUCCESS) {
  ...
  if (found) {
    ...
    if (maxOpNum <= 0) {
      ACL_LOG_INNER_ERROR(
          "[Check][MaxOpNum]max_opqueue_num [%s] is invalid from buffer, "
    ...
    if (static_cast<uint64_t>(maxOpNum) < DEFAULT_MAX_OPQUEUE_NUM) {
      ACL_LOG_WARN(
          "max_opqueue_num [%lu] is less than default value DEFAULT_MAX_OPQUEUE_NUM[%lu], "
          "it may be low performance.",
          static_cast<uint64_t>(maxOpNum), DEFAULT_MAX_OPQUEUE_NUM);
    }
    opModels_.SetMaxOpNum(static_cast<uint64_t>(maxOpNum));
  } else {
    ACL_LOG_INFO("no max_opqueue_num found, set default DEFAULT_MAX_OPQUEUE_NUM[%lu].", DEFAULT_MAX_OPQUEUE_NUM);
  }
...
aclError AclOpResourceManager::RegisterModel(OpModelDef &&modelConfig, ModelMap &opModelDefs, const bool isDynamic,
                                             bool &isRegistered, const bool isStaticRegister) {
...
aclError AclOpResourceManager::SetHostMemToConst(const AclOp &aclopHostMemToConst, bool &isExistConst) const {
  for (int32_t i = 0; i < aclopHostMemToConst.numInputs; ++i) {
    if ((aclopHostMemToConst.inputDesc[i]->memtype == ACL_MEMTYPE_HOST) &&
      ...
      ACL_REQUIRES_OK(
          SetTensorConst(const_cast<aclTensorDesc *>(aclopHostMemToConst.inputDesc[i]), aclopHostMemToConst.inputs[i]));
    }
  }
...
aclError AclOpResourceManager::MatchStaticOpModel(const AclOp &aclOp, OpModel &opModel, bool &isDynamic,
                                                  bool &isNeedMatchDymaic) {
  ...
  if (isExistConst) {
    ...
    if (ret == ACL_SUCCESS) {
      isDynamic = false;
      ACL_LOG_INFO("Match static model with const memory successfully. opType = %s, opModel = %s", aclOp.opType.c_str(),
                   modelDef->modelPath.c_str());
      ...
      if (ret == ACL_SUCCESS) {
        isDynamic = false;
        ACL_LOG_INFO("Match static model successfully. opType = %s, opModel = %s", aclOp.opType.c_str(),
                     modelDef->modelPath.c_str());
        ret = modelCache_.GetOpModel(*modelDef, opModel);
        return ret;
      }
...
aclError AclOpResourceManager::MatchDynamicOpModel(const AclOp &aclOp, OpModel &opModel, bool &isDynamic) {
  ...
  for (size_t i = 0U; i < opRangeInfoVec->size(); ++i) {
    ...
    if (ret == ACL_SUCCESS) {
      ACL_LOG_INFO("Match dynamic model success. opType = %s, opModel = %s", aclOp.opType.c_str(),
                   modelDef->modelPath.c_str());
      isDynamic = true;
      ret = modelCache_.GetOpModel(*modelDef, opModel);
      return ret;
...
aclError AclOpResourceManager::MatchOpModel(const AclOp &aclOp, OpModel &opModel, bool &isDynamic) {
  ...
  if (UNLIKELY((ret != ACL_SUCCESS) && (!aclOp.isCompile))) {
    ACL_LOG_INNER_ERROR(
        "[Match][OpModel]MatchOpModel opName = %s fail from static map or dynamic map. "
...
aclError AclOpResourceManager::ReadModelDefs(const std::string &configPath, std::vector<OpModelDef> &configList) {
...
aclError AclOpResourceManager::CreateRT2Executor(std::shared_ptr<gert::StreamExecutor> &streamExecutor,
                                                 rtStream_t stream, const gert::ModelExecuteArg &arg,
                                                 gert::ModelV2Executor *&executor) {
