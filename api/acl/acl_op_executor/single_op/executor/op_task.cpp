...
TbeOpTask::TbeOpTask(const void *const stubFunction, const uint32_t block)
    : OpTask(), stubFunc_(stubFunction), blockDim_(block) {}

aclError TbeOpTask::ExecuteAsync(const int32_t numInputs, const aclDataBuffer *const inputs[], const int32_t numOutputs,
  ...
  const auto ret = rtKernelLaunch(stubFunc_, blockDim_, const_cast<uint8_t *>(args_.get()), argSize_, nullptr, stream);
