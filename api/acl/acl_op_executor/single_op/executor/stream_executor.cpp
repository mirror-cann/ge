...
aclError GetAlignedAndPaddingSize(const size_t size, const bool isPadding, size_t &alignedSize) {
  ...
  const size_t appendSize = isPadding ? (DATA_MEMORY_ALIGN_SIZE + padding_size) : DATA_MEMORY_ALIGN_SIZE;
...
aclError StreamExecutor::ExecuteAsync(const AclOp &aclOpDesc, const aclDataBuffer *const *const inputs,
...
aclError StreamExecutor::ExecuteAsync(OpKernelDesc &kernelDesc, const int32_t numInputs,
                                      const aclDataBuffer *const *const inputs, const int32_t numOutputs,
                                      aclDataBuffer *const *const outputs) {
...
aclError StreamExecutor::InitTbeTask(const OpKernelDesc &desc, const int32_t numInputs, const int32_t numOutputs,
                                     TbeOpTask &task) {
  // create new op task
  const size_t numWorkSpaces = desc.workspaceSizes.size();
  if (numWorkSpaces > MAX_WORKSPACES) {
    ACL_LOG_INNER_ERROR(
        "[Check][numWorkSpaces]numWorkSpaces invalid, "
...
aclError StreamExecutor::AllocateWorkspaces(const std::vector<size_t> &workspaceSizes,
  ...
  if (numWs > MAX_WORKSPACES) {
    ACL_LOG_INNER_ERROR("[Check][numWs]numWs invalid, numWs[%zu] is larger than MAX_WORKSPACES[%zu]", numWs,
                        MAX_WORKSPACES);
    return ACL_ERROR_INVALID_PARAM;
  }
