...
void AclResourceManagerOm2::AddOm2Executor(uint32_t &modelId, std::unique_ptr<gert::Om2ModelExecutor> &&executor) {
...
aclError AclResourceManagerOm2::GetOpDescInfo(uint32_t deviceId, uint32_t streamId, uint32_t taskId,
  ...
  ACL_LOG_INFO("[OM2][Get][OpDescInfo] cannot find op desc info, deviceId[%u], streamId[%u], taskId[%u]", deviceId,
               streamId, taskId);
