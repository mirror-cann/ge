...
#define ACL_LOG_EVENT(fmt, ...)                                                                                        \
  do {                                                                                                                 \
    if (acl::AclLog::IsEventLogOutputEnable()) {                                                                       \
      constexpr const char_t *const funcName = __FUNCTION__;                                                           \
      dlog_info((acl::ACL_MODE_ID | (RUN_LOG_MASK)), "%d %s: " fmt, acl::AclLog::GetTid(), acl::format_cast(funcName), \
                ##__VA_ARGS__);                                                                                        \
    }                                                                                                                  \
  } while (false)
#endif
...
#define ACL_CHECK_RANGE_INT(val, min, max)                                                                        \
  do {                                                                                                            \
    if (((val) < (min)) || ((val) > (max))) {                                                                     \
      ACL_LOG_ERROR("[Check][%s]param:[%d] must be in range of [%d] and [%d]", #val, (static_cast<int32_t>(val)), \
...
#define ACL_CHECK_RANGE_FLOAT(val, min, max)                                                                      \
  do {                                                                                                            \
    if (((val) < (min)) || ((val) > (max))) {                                                                     \
      ACL_LOG_ERROR("[Check][%s]param:[%.2f] must be in range of [%.2f] and [%.2f]", #val,                        \
                    (static_cast<float64_t>(val)), (static_cast<float64_t>(min)), (static_cast<float64_t>(max))); \
      return ACL_ERROR_INVALID_PARAM;                                                                             \
    }                                                                                                             \
  } while (false)
  ...
  do {                                                                                             \
    if ((val) < 0) {                                                                               \
      ...
      acl::AclErrorLogManager::ReportInputError(                                                   \
          "EH0001", std::vector<const char *>({"param", "value", "reason"}),                       \
