...
static int32_t RegularFileFilterFn(const mmDirent2 *const entry) {
  ...
  const bool isFile =
      (static_cast<int32_t>(entry->d_type) == MM_DT_REG) || (static_cast<int32_t>(entry->d_type) == DT_UNKNOWN);
...
aclError ListFiles(const std::string &dirName, FileNameFilterFn filter, std::vector<std::string> &names,
  ...
  for (int32_t i = 0; i < ret; ++i) {
    ...
    // some xfs file system may return DT_UNKNOWN
    const bool isFile =
        (static_cast<int32_t>(dirEnt->d_type) == MM_DT_REG) || (static_cast<int32_t>(dirEnt->d_type) == DT_UNKNOWN);
