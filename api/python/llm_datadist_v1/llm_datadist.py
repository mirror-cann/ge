...
from .configs import LLMClusterInfo, LLMRole
...
from .utils.utils import check_int32, check_isinstance, check_uint64
...
class LLMDataDist(object):
    ...
    def _check_flow_graph_max_size(self, options: Dict[str, str]) -> None:
        ...
        raise_if_false(
            len(value.split(",")) == 1,
            "ge.flowGraphMemMaxSize only support one mem pool in llm datadist",
        )
    ...
    def init(self, options: Dict[str, str]) -> None:
        ...
        if self._is_initialized:
            return
        raise_if_false(
            LLMDataDist.llm_engine_instance is None,
            "Cannot init multiple LLM engines",
        ...
        self._enable_cache_mgr = (
            "llm.EnableCacheManager" in self._engine_options and self._engine_options["llm.EnableCacheManager"] == "1"
        )
    ...
    def unlink_clusters(
        self,
        clusters: Union[List[LLMClusterInfo], Tuple[LLMClusterInfo]],
        timeout=3000,
        force=False,
    ):
        self._check_is_inited()
...
def _shutdown_handler():
    if LLMDataDist.llm_engine_instance is not None:
        ...
        try:
            ...
            log.warn(
                f"error occurred while finalize llm datadist: {e} may cause by already finalized by another framework"
            )
