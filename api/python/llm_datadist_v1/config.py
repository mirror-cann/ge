...
from .configs import LlmConfig, LLMRole, trans_str_ip
...
class ClusterConfig:
    ...
    def _parse_rank_mapping(engine_options: Dict[str, str]) -> Dict[str, int]:
        raise_if_false(
            "llm.ClusterInfo" in engine_options,
            "option 'llm.ClusterInfo' is not defined",
        )
    ...
    def _parse_numa_config(engine_options) -> Dict[str, int]:
        raise_if_false(
            "ge.resourceConfigPath" in engine_options,
            "option 'ge.resourceConfigPath' is not defined",
        )
...
class EngineConfig:
                    ...
                    "cluster_nodes": [
                        {
                            ...
                            "port": -1,
                            "is_local": True,
                            "data_panel": {"avail_ports": "65000~65535"},
                            "item_list": item_list,
                        }
                    ],
                }
    ...
    def gen_cluster_info_if_not_exist(cluster_id: int, role: LLMRole, engine_options: Dict[str, str]) -> None:
        ...
        if role == LLMRole.PROMPT:
            raise_if_false(
                "llm.listenIpInfo" in engine_options,
            ...
            raise_if_false(
                len(sub_ip_infos) == device_num,
                f"listen ip info num:{len(sub_ip_infos)} in llm.listenIpInfo is not equal to device num:{device_num}.",
            )
        ...
        if "RESOURCE_CONFIG_PATH" in os.environ:
            ...
            raise_if_false(
                "ge.exec.deviceId" in engine_options,
            ...
            raise_if_false(
                len(device_id.split(";")) > 0,
                f'ge.exec.deviceId is invalid, value="{device_id}", At least one device id is required.',
            )
