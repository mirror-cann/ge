...
def to_data_type(data_type_str: str) -> DataType:
    for dtype in DataType:
        if str(dtype.value) == data_type_str:
            return dtype
    raise LLMException(
        f"unsupported dtype: {data_type_str}",
        status_code=LLMStatusCode.LLM_PARAM_INVALID,
    )
...
def calc_tensor_size(shape: List[int], data_type: DataType) -> int:
    ...
    raise_if_false(
        tensor_size >= 0,
        "Failed to calc tensor size, shape = {0}, data_type = {1}",
        shape,
        data_type,
    )
    return tensor_size
...
class CacheDescParser(object):
    ...
    def parse_by_options(options: Dict[str, str]) -> CacheDesc:
        ...
        if "llm.RefInputSeqLenDimIndex" in options:
            ...
            seq_len_dim_index = -1
            log.info(
                f"llm.RefInputSeqLenDimIndex not set and can not infer by shape,seq_len_dim_index = {seq_len_dim_index}"
            )
...
def build_npu_tensors(kv_cache: KvCache, device_index: int = 0) -> List[int]:
    return llm_wrapper.build_npu_tensors(
        kv_cache.cache_desc.shape,
...
def pack_cache_desc(cache_desc: CacheDesc) -> Tuple[int, int, int, List[int], int, int]:
    return (
        cache_desc.num_tensors,
        cache_desc.data_type.value,
        cache_desc.seq_len_dim_index,
        cache_desc.shape,
        cache_desc.placement.value,
        cache_desc._is_blocks,
    )
...
def pack_cache_key_by_id(
    cache_key: CacheKeyByIdAndIndex,
) -> Tuple[int, int, int, int, int, int, bool]:
    return (
        cache_key.cluster_id,
        cache_key.cache_id,
        cache_key.batch_index,
        _INVALID_ID,
        _INVALID_ID,
        0,
        False,
...
def pack_block_cache_key(
    cache_key: BlocksCacheKey,
) -> Tuple[int, int, int, int, int, int, bool]:
    return (
        cache_key.prompt_cluster_id,
        -1,
...
class TransferCacheParameters:
    ...
    transfer_configs: Union[
        List[Union[TransferConfig, TransferWithCacheKeyConfig]],
...
class TransferCacheJob:
    ...
    def init(self):
        raise_if_false(
            self._cache_desc.num_tensors % 2 == 0,
            "cache_desc.num_tensors ({0}) is not even",
    ...
    def transfer_layers(self):
        for layer_i, src_layer_index in enumerate(range(self._num_layers)):
            ...
            for config in to_transfer:
                dst_layer_index = (
                    src_layer_index if config.dst_layer_range is None else list(config.dst_layer_range)[layer_i]
                )
    ...
    def transfer_layer(
        ...
        src_layer_index: int,
        dst_layer_idx,
        transfer_config: Union[TransferConfig, TransferWithCacheKeyConfig],
    ) -> LLMStatusCode:
        if check_type(transfer_config, TransferConfig):
            dst_layer_index = src_layer_index - transfer_config.src_layer_range.start
            dst_addrs = transfer_config.dst_addrs[
                dst_layer_index * _NUM_TENSORS_PER_LAYER : dst_layer_index * _NUM_TENSORS_PER_LAYER
            ...
            transfer_config = (
                ...
                src_layer_index,
                dst_addrs,
                transfer_config.dst_cluster_id,
                0,
                0,
                PushType.NO_CACHE_KEY.value,
                src_layer_index,
                2,
            )
        else:
            if check_type(transfer_config.cache_key, BlocksCacheKey):
                transfer_config = (
                    ...
                    src_layer_index,
                    [],
                    transfer_config.cache_key.cluster_id,
                    transfer_config.cache_key.model_id,
                    0,
                    PushType.BLOCKS_CACHE_KEY.value,
                    dst_layer_idx,
                    2,
                )
                ...
                transfer_config = (
                    ...
                    [],
                    transfer_config.cache_key.cluster_id,
                    transfer_config.cache_key.cache_id,
                    transfer_config.cache_key.batch_index,
                    PushType.CACHE_KEY_BY_ID.value,
                    dst_layer_idx,
                    2,
                )
        block_config = (
            self._dst_block_memory_size if self._dst_block_memory_size is not None else 0,
    ...
    def check_transfer_config(self, transfer_config: Union[TransferConfig, TransferWithCacheKeyConfig]):
        if self._src_block_indices:
            raise_if_false(
                transfer_config.src_batch_index == 0,
                "Invalid TransferConfig, src_batch_index ({0}) != 0 while src is blocks",
                transfer_config.src_batch_index,
            )
            raise_if_false(
                0 <= transfer_config.src_batch_index < self._cache_desc.batch_size,
                "Invalid TransferConfig, src_batch_index ({0}) out of range: [0, {1})",
                transfer_config.src_batch_index,
                self._cache_desc.batch_size,
            )
        raise_if_false(
            ...
            transfer_config.src_layer_range,
            self._num_layers,
        )
        num_tensors_to_transfer = (
            transfer_config.src_layer_range.stop - transfer_config.src_layer_range.start
        ) * _NUM_TENSORS_PER_LAYER
        if check_type(transfer_config, TransferConfig):
            raise_if_false(
                len(transfer_config.dst_addrs) == num_tensors_to_transfer,
                "expect {0} dst_addrs, but len(dst_addrs) = {1}, range = {2}",
                num_tensors_to_transfer,
                len(transfer_config.dst_addrs),
                transfer_config.src_layer_range,
            )
...
def transfer_cache_async(
    ...
    if params.src_block_indices:  # src is blocks
        ...
        raise_if_false(
            ...
            len(params.src_block_indices),
            len(params.dst_block_indices),
        )
    else:  # src is cache
        raise_if_true(
            (transfer_cache_func == llm_wrapper.transfer_cache) and (params.dst_block_indices is not None),
            "transfer from cache to blocks is not supported",
        )
        if params.dst_block_indices:
            raise_if_false(
                params.dst_block_memory_size is not None,
    ...
    if not enable_remote_cache:
        ...
        check_isinstance(
            ...
            [list, tuple],
            TransferWithCacheKeyConfig,
            "While enable_remote_cache_accessible is True, ",
            allow_none=False,
        )
    raise_if_false(
        params.dst_block_indices or params.dst_block_memory_size in (None, 0),
...
def layer_range_to_tensor_indices(src_layer_range: range, dst_layer_range: range, tensor_num_per_layer: int = 2):
    ...
    raise_if_true(
        (src_layer_range is not None) and (src_layer_range.step != 1),
        "param check failed, src_layer_range step must be 1.",
    )
    raise_if_true(
        (dst_layer_range is not None) and (dst_layer_range.step != 1),
    ...
    if len(src_layer_indices) != 0:
        raise_if_true(
            src_layer_indices[0] < 0,
            "src_layer_range is invalid, the start value:{0} is < 0",
            src_layer_indices[0],
        )
    ...
    if len(dst_layer_indices) != 0:
        raise_if_true(
            dst_layer_indices[0] < 0,
            "dst_layer_range is invalid, the start value:{0} is < 0",
            dst_layer_indices[0],
        )
