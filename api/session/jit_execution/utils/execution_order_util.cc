...
Status ExecutionOrderUtil::GetGuardedExecutionPointGraphKey(const GuardedExecutionPoint *gep,
                                                            std::string &gep_graph_key) {
  return ep_util_.GetGuardedExecutionPointGraphKey(gep, gep_graph_key);
}

Status ExecutionOrderUtil::CreateKeyOptionForGuardedExecutionPoint(const std::string root_dir,
...
Status ExecutionOrderUtil::RestoreExecutionOrder(const std::string root_dir, const std::string user_graph_key,
  ...
  GE_CHK_BOOL_RET_STATUS(ModelCache::CheckFileExist(slicing_result_file_path), FAILED,
                         "Cannot find the slicing result file of the user graph key: %s.", user_graph_key.c_str());
  ...
  for (size_t i = 0; i < ep_count; ++i) {
    ...
    const std::map<std::string, std::string> &ep_options = (order.slice_graphs_.empty() ? order.first_ep_options_
                                                            : (i == ep_count - 1)       ? order.last_ep_options_
                                                                                        : order.middle_ep_options_);
    GE_WARN_ASSERT_GRAPH_SUCCESS(
        ep_util_.RestoreExecutionPoint(root_dir, user_graph_key, info, ep_options, exec_point_ptr),
...
Status ExecutionOrderUtil::SaveExecutionOrder(const std::string root_dir, const std::string user_graph_key,
                                              uint32_t user_graph_id, ExecutionOrder &order) {
