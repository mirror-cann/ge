...
static Status SaveGepListJsonFile(const std::string &gep_list_file,
                                  const GuardedExecutionPointInfoList &gep_info_list) {
...
Status GuardedExecutionPointUtil::AddGuardedExecutionPointGraphKey(std::string root_dir, std::string user_graph_key,
  ...
  GE_CHK_BOOL_RET_STATUS(gep_to_gep_graph_key_.find(gep) == gep_to_gep_graph_key_.end(), FAILED,
                         "The GEP has been added to CompiledModelCache.");
  const std::string cur_timestamp = GetCurTimeInNs();
  std::string gep_graph_key = user_graph_key + "_" + std::to_string(gep->GetOwnerEp()->GetId()) + "_" + cur_timestamp;
...
Status GuardedExecutionPointUtil::EmplaceGuardedExecutionPointOption(std::string root_dir, std::string user_graph_key,
                                                                     const GuardedExecutionPoint *gep,
  ...
  GE_ASSERT_NOTNULL(gep);
  GE_CHK_BOOL_RET_STATUS(options.find(OPTION_GRAPH_KEY) == options.end(), FAILED,
                         "Current options already have the graphKey option.");
  GE_CHK_BOOL_RET_STATUS(options.find(OPTION_GRAPH_COMPILER_CACHE_DIR) == options.end(), FAILED,
                         "Current options already have the cache_dir option.");
...
Status GuardedExecutionPointUtil::HasGuardedExecutionPointRegistered(const GuardedExecutionPoint *gep,
                                                                     bool &has_registered) {
  GE_ASSERT_NOTNULL(gep);
  has_registered = true;
  const std::lock_guard<std::mutex> lock(gep_util_mutex_);
...
Status GuardedExecutionPointUtil::GetGuardedExecutionPointGraphKey(const GuardedExecutionPoint *gep,
                                                                   std::string &gep_graph_key) {
  const std::lock_guard<std::mutex> lock(gep_util_mutex_);
  GE_ASSERT_NOTNULL(gep);
  const auto gep_iter = gep_to_gep_graph_key_.find(gep);
  GE_CHK_BOOL_RET_STATUS(gep_iter != gep_to_gep_graph_key_.end(), FAILED,
                         "Get GEP graph key failed. Cannot find the GEP in the CompiledModelCache.");
  gep_graph_key = gep_iter->second;
  return SUCCESS;
}

Status GuardedExecutionPointUtil::RestoreGuardedExecutionPoint(const std::string root_dir,
                                                               const GuardedExecutionPointInfo &info,
                                                               ExecutionPoint &exec_point, GuardedExecutionPoint *gep) {
  /* Reconstruct the gep_to_gep_graph_key mapping */
  GE_CHK_BOOL_RET_STATUS(gep_to_gep_graph_key_.find(gep) == gep_to_gep_graph_key_.end(), FAILED,
                         "Failed to restore gep_to_gep_graph_key, the gep has existed.");
   ...
   * & flow_model_cache init, at the same, restore cache also need update the graph option before using
   * flow_model_cache.
   **/
   ...
   * 2025.5.28
   * todo:
   * compiled_设成false，在jit_executor中对该gep执行AddGraph和CompileGraph操作，注册graphNode以及从om中读取FlowModel。
   * 优点：
...
Status GuardedExecutionPointUtil::SaveGuardedExecutionPoint(
    std::string user_graph_root_dir, const ExecutionPoint &exec_point,
  ...
  GE_CHK_STATUS_RET(SaveGepListJsonFile(gep_list_file, gep_info_list), "Failed to save gep list json file[%s].",
                    gep_list_file.c_str());
