...
目录结构：`master/{build_date}/{arch}/Ascend-cann-toolkit_{version}_linux-{arch}.run`

**注意**：
- 构建日期（如 `20260401000324385`）会动态变化，需要实时获取
- 完整 URL 格式：`https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/{build_date}/{arch}/`

### 3. 下载 Toolkit 包
```bash
# 使用 wget 下载（支持进度显示）
wget -O "Ascend-cann-toolkit_${version}_linux-${arch}.run" \
  "https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/${latest_date}/${arch}/Ascend-cann-toolkit_${version}_linux-${arch}.run" \
  --progress=bar:force
```
